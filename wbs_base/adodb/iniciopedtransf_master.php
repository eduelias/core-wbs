...
$prod = new operacao();
	...
	$prod->listProd("seguranca", "arquivo='iniciopedtransffpg_master.php'");
	...
	#$prod->listProd("seguranca", "arquivo='inicioclienteform.php'");
		...
		if (!$codemptransfselect){$codemptransfselect = 1;}
	...
	");
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			if ($puc_prod == 0){
				...
				$pcu_oc = $prod->showcampo0();
				$ppu  = $pcu_oc;
				$ppuf = number_format($pcu_oc*1.02,2,',','.'); 
			}
...
if ($codgrp_vend == 2 or $codgrp_vend == 20 or $codgrp_vend == 64){
...
/// CONTADOR DE PAGINAS ////
$Action= "update";
$compl= "codpednew=$codpednew&codclienteselect=$codclienteselect&codempselect=$codempselect&codemptransfselect=$codemptransfselect&codvendselect=$codvendselect&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport#adicionar";   
