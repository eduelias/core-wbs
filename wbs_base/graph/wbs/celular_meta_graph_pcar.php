...
if ($codproj == -1){
 ...
 for($i = 0; $i <= 31; $i++ ){
	 if (strlen($i)==1){$u = '0'.$i;}else{$u = $i;}
         ...
         // PLANO CARTAO
		$prod->clear();
        $prod->listProdU("codplano, sum(qtde)", "pedido, pedidoprod", "pedido.codped=pedidoprod.codped  and dataped like '$ano-$u1-$u%' AND codplano = 7 AND cancel <> 'OK' ". $cond ." GROUP by codplano");
