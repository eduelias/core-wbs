...
if ($codproj == -1){
 ...
 for($i = 0; $i <= 31; $i++ ){
	 if (strlen($i)==1){$u = '0'.$i;}else{$u = $i;}
         ...
         // PLANO CARTAO
		$prod->clear();
        $prod->listProdU("codplano, sum(qtde), sum(vpv)", "pedido, pedidoprod", "pedido.codped=pedidoprod.codped  and dataped like '$ano-$u1-$u%' AND codplano = 7 AND cancel <> 'OK' ". $cond ." GROUP by codplano");
        $pcar_d[$i] = $prod->showcampo2();
        $pcar_a = $pcar_a + $pcar_d[$i];

        if ($i > $diahoje){$pcar_a = 0;}
...
$graph->title->Set("FATURAMENTO CARTÃO - $mespesq $ano ");
$graph->subtitle->Set("");
