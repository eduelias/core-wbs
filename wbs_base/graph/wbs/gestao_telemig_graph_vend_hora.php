...
$prod = new operacao();
 ...
 for($i = 0; $i <= 24; $i++ ){
	 if (strlen($i)==1){$u = '0'.$i;}else{$u = $i;}
		...
		$prod->clear();
        $prod->listProdU("SUM(if (codplano = 4,qtde, 0)) as controle, SUM(if (codplano = 16, qtde, 0)) as controle_120, SUM(if (codplano = 17, qtde, 0)) as controle_160, SUM(if (codplano = 5,qtde, 0)) as ideal, SUM(if (codplano = 6, qtde, 0)) as ideal_micro, SUM(if (codplano = 13, qtde, 0)) as ideal_75, SUM(if (codplano = 14, qtde, 0)) as ideal_150, SUM(if (codplano = 15, qtde, 0)) as ideal_300, SUM(if (codplano = 7, qtde, 0)) as cartao, SUM(if (codplano = 18,qtde, 0)) as controle80, SUM(if (codplano = 19,qtde, 0)) as ideal600", "pedido, pedidoprod", "pedido.codped=pedidoprod.codped  and dataped like '$ano-$u1% $u%' AND codvend = '$codvend_select' AND cancel <> 'OK' ". $cond);
...
$graph->title->Set("PLANO IDEAL + CONTROLE + CARTÃO - $mespesq $ano ");
$graph->subtitle->Set("");
