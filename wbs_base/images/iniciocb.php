    ...
    case "insert":
		...
		if ($codbarra[$i] <> ""){
			...
			if ($numreg == 0 and $ins[$i] == 0){
				...
				$prod->listProd("ocprod", "codoc=$codoc and codprod = $codprod");
		...
		for($i = 0; $i < count($array99); $i++ ){
			...
			$libcb = $prod->showcampo0();
				...
				*/
				...
				$prod->listProd("ocprod", "codoc=$codoc and codprod=$codprod");
				$prod->setcampo11("OK");
				$prod->upProd("ocprod", "codoc=$codoc and codprod = $codprod");
...
for($i = 0; $i < $cont; $i++ ){
	$j=$i+1;
	$prod->clear();
	$prod->listProdU("codbarra", "codbarra", "codprod=$codprod and codemp=$codemp and codoc=$codoc and flag_ins = $i");
	$cb = $prod->showcampo0();
	if ($cb <> ""){
		$ne[$i] = 1;
		$ins[$i] = 1;
