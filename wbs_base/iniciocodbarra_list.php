...
$prod->listProd("vendedor", "nome='$login'");
    	...
    	#if (($codgrp_transf == 32 or $codgrp_transf == 53) and $codemp_transf <> 0){$block_transf = 1;$allemp = 'N';$codempvend = $codemp_transf;$codempselect = $codempvend;}
...
if ($Actionsec == "list"){
		...
		//PESQUISA POR COD PÇRODUTO
		if ($codpesq){
							
			$condicao3 .= " estoque.codprod='$codpesq' and ";
		}
		...
		$prod->listProdSum("estoque.codest, estoque.codemp, estoque.codprod,  estoque.qtde, estoque.reserva, estoque.estoquemin, produtos.nomeprod, produtos.hist, produtos.unidade, chkcb  ", "estoque, produtos", $condicao3, $array, $array_campo, $parm );
...
<tr bgcolor='#93BEE2'> 
    <td width='5%' height='0' bgcolor='#FFFFFF'>&nbsp;</td>
    <td width='80%' height='0' bgcolor='#FFFFFF'>&nbsp;</td>
     <td align='center' width='5%' height='0'  bgcolor='#FFFFFF'> 
      <div align='center'><b><font color='#0000FF' size='1' face='Verdana, Arial, Helvetica, sans-serif'>P</font><font face='Verdana, Arial, Helvetica, sans-serif' size='1'><br />
        <font color='#FF0000'>V</font></font></b></div>    </td>
    <td align='center' width='5%' height='0' bgcolor='#FFFFFF'>&nbsp;</td>
	  <td align='center' width='5%' height='0' bgcolor='#FFFFFF' >&nbsp;</td>
  </tr> 
	<tr bgcolor='#93BEE2'> 
    <td width='5%' height='0'> 
    ...
    <td width='80%' height='0'> 
     ...
     <td align='center' width='5%' height='0'  bgcolor='#F7BBC4'> 
      <div align='center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>EST<br> FIS</font></b></div>
    </td>
    <td align='center' width='5%' height='0' bgcolor='#C7E3C6'> 
      <div align='center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>EST<BR>SIS</font></b></div>
    </td>
	  <td align='center' width='5%' height='0' bgcolor='#E2E2E2' > 
      <div align='center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>RES<BR>SIS</font></b></div>
    </td>
...
if (count($array) <> 0){
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			$prod->listProdU("SUM( if( tipo_fab = 'P', 1, 0 ) ) as P, SUM( if( tipo_fab != 'P', 1, 0 ) ) as V", "codbarra", "codprod = $codprod and codemp=$codempselect and codbarraped <> 1 ");
			$qtdebarra_venda = $prod->showcampo1();
			$qtdebarra_prod = $prod->showcampo0();
			
			#if (!$qtdebarra_prod){$qtdebarra_prod = 0;}
			#if (!$qtdebarra_venda){$qtdebarra_venda = 0;}
			
			/*				
			// VERIFICA QUANTIDADES USADAS DESSE PRODUTO
			...
			*/
			...
			$prod->listProdSum("codbarra, flag_acerto, tipo_fab", "codbarra", "codprod = $codprod and codemp=$codempselect and codbarraped <> 1", $array1, $array_campo1, "" );

			#$est_real = ($qtdebarra_insert)-$qtdevend_usado;
			#$est_real = ($qtdebarra_insert);
...
echo(" 
	<tr bgcolor='$cor'> 
      ...
      <div align='center'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>$codprod</font></div>
		...
		<td width='80%' height='0'> 
      <div align='left'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'><b>$nomeprod</B></font>
	    <hr size='1'>
");
 for($j = 0; $j < count($array1); $j++ ){
			...
			$flag_acerto = $prod->showcampo1();
			$tipo_fab = $prod->showcampo2();
			
			if ($tipo_fab == "P"){$cor1 ="#0000FF";}else{$cor1 ="#FF0000";}

if (($codgrp_vend <> 41 and $codgrp_vend <> 65) or $chkcb <> 'Y' ){


if ($flag_acerto <> 'N' AND $flag_acerto <> 'X'){
echo("
...
} //BLOQUEIA CAIXAS E GERENTES
 }
 echo("	
	
	</div>
		  <td align='center' width='5%' height='0' bgcolor='#F7BBC4'> 
      <div align='center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1' color = '#0000FF'>$qtdebarra_prod<BR><font color='#FF0000'>$qtdebarra_venda</font></font></b></div>
		  </td>
		 <td align='center' width='5%' height='0' bgcolor='#C7E3C6'> 
...
echo("
<tr bgcolor='$cor'> 
			 <td width='10%' height='0' colspan = '6' align = 'center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1' color='#FF0000'><br>NENHUM PRODUTO ENCONTRADO</font></b></td>
	</tr>
");
}
