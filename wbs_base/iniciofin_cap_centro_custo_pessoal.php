...
$nomeform = "PEDIDO";
$titulo = "CENTRO DE CUSTOS PESSOAL";
...
switch ($Action) {
	...
	case "inscentrocusto":
			...
			$soma = $soma_pt;
			#$somar = ($soma+$valornew);
			#echo("s=$soma - $valornew - $somar - $valortotal");
	 ...
	 case "inscentrocusto_pessoal":
		...
		$prod->listProdSum("codvend, codproj ","vendedor", "block <>'Y' and funcionario = 'Y'", $array, $array_campo , "order by nome");
		for($i = 0; $i < count($array); $i++ ){
			...
			if ($valornew[$cv] <> ""){
				...
				#$somar = $soma+$valornew[$cv];
				#echo("s=$soma - $valornew[$cv] - $somar - $valortotal");
	...
	case "delcentrocusto":
	 ...
	 case "newreg":
		...
		$codcapselect = $uregcap;
			...
			$prod->clear();
			$prod->listProdgeral("projeto_relacao_custo","codcap='$codcapselect'", $array45, $array_campo45, "" );
			for($uy = 0; $uy < count($array45); $uy++ ){
				$prod->mostraProd( $array45, $array_campo45, $uy );
				$codcusto = $prod->showcampo0();

			$prod->setcampo7($valorp);
			$prod->upProd("projeto_relacao_custo", "codcusto=$codcusto");
...
if ($Actionsec == "list"){
		...
		if ($codempselect <> ""):
			...
			#$condicao3 .= "(opcaixa like '800.%' or opcaixa like '810.%') and ";
			if ($hist == "N"){
				$condicao3 .= "hist = 'N'";
			}else{
				$condicao3 .= "hist = 'S'";
			...
			$prod->listProdSum("datavenc, numdoc, opcaixa, codfor, valordevido, hist, descricao, nf, codcap, codoc , datapg, valorpago, juros, obs, previsao", "fin_cap", $condicao3, $array, $array_campo, $parm );
...
echo("										
 ...
 <tr>
                                ...
                                R$ <input type='text' name='valornew' size='12' onKeyUp='this.value = adjust(this);'>
...
");
	$prod->listProdSum("codvend, nome, tipocliente, doc","vendedor", "block <>'Y' and funcionario = 'Y'", $array, $array_campo , "order by nome");
	for($i = 0; $i < count($array); $i++ ){
  ...
  <tr>
	...
	");
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			#$bgcorlinha="#E5E5E5";
			#if ($hist == "N"){$bgcorlinha="#F2E4C4";}
			if ($previsao == "S"){$bgcorlinha = "#FFEC9F";}else{$bgcorlinha="#F3F8FA";}
			if ($hist == "S"){$bgcorlinha="#E1AFAA";}
...
echo("
<a
href='$PHP_SELF?Action=update&codcapselect=$codcap&codcxselect=$codcxselect&codcxsaldoselect=$codcxsaldoselect&codempselect=$codempselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&retlogin=$retlogin&connectok=$connectok&pg=$pg&hist=$hist&codforselect=$codforselect		&opcaixaselect=$opcaixaselect&dde=$dde&mde=$mde&ade=$ade&date=$date&mate=$mate&aate=$aate#contapg'>Detalhes</a>
");
}
echo("</b></font></td>
<td width='10%' height='0' align='center'><font face='Verdana, Arial, Helvetica, sans-serif'
...
if ($Action == "newreg"){
	...
	if ($codcapselect <> ""){
			...
			$xprevisao = $prod->showcampo19();
			if ($xprevisao == "S"){$cor = "#FFEC9F";$chk = "checked";}else{$cor = "#D6E7EF";$chk = "";}
						 ...
						 <tr> 
						  ...
						  <tr bgcolor='$cor'>
                            ...
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='2'>
                               ...
                               <input type='text' name='dvenc' value= '$xdvenc' size='2' maxlength='2'>/ <input type='text' name='mvenc' value= '$xmvenc' size='2' maxlength='2'>/ <input type='text' name='avenc' value= '$xavenc' size='4' maxlength='4'>&nbsp;&nbsp;&nbsp;<input type='checkbox' name='previsao' value='S' $chk><b>PREVISÃO</b>
