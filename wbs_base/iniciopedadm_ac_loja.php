...
if ($Actionsec == "list"){
		...
		$prod->listProdSum("codped, pedido.codcliente, pedido.codvend, dataped, dataprevent, status, horaprevent, nf, contrato, libentr, codbarra, caixa, clientenovo.nome, pendfpg, reavalfpg, fat, modped", "pedido, clientenovo, vendedor", $condicao3, $array, $array_campo, $parm );
...
if ($modped == "OK"){$mod ="MOD";}else{$mod="";}
		...
		if ($parc_visa > 0  ){$cartao = "1";}else{$cartao="0";}
		if ($parc_master > 0 ){$cartao1 = "1";}else{$cartao1="0";}
...
size='1'><b>
");
if ($cartao == 0 and $cartao1 == 0){
echo("
<a
href='$PHP_SELF?Action=update&codped=$codped&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&retlogin=$retlogin&connectok=$connectok&pg=$pg&hist=$hist&nomevendpesq=$nomevendpesq&codpedpesq=$codpedpesq'>$codbarra</font></b></a>
");
}else{
echo ("
$codbarra
