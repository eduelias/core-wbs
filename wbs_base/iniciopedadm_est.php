...
if ($Actionsec == "list"){
		...
		$condicao4 = " (pedido.status='APROV' or pedido.status='PECA' or (pedido.modped = 'OK' and pedido.cb = 'NO')) and";
		...
		$prod->listProdSum("codped, pedido.codcliente, pedido.codvend, dataped, dataprevent, status, horaprevent, nf, contrato, libentr, codbarra, caixa, clientenovo.nome, modped, modoentr, dataprevent_old, declara", "pedido, clientenovo, vendedor", $condicao3, $array, $array_campo, $parm );
  ...
  <center>
    ...
    <tr>
     ...
     <td width='304' bgcolor='#F0F0F0' valign='top' align='right'><font color='#336699' face='Verdana, Arial, Helvetica, sans-serif' size='1'><b>PREVISÃO
        ENTREGA:<br>
        </b></font><font face='Verdana, Arial, Helvetica, sans-serif' color='#000000' size='4'><b>$datapreventf - $horaprevent</b></font><BR>
...
<table border='0' width='550' border='0' cellspacing='1' cellpadding='2' align='center'>
  <tr bgcolor='#D6B778'> 
	...
	<td width='80%' height='0' colspan='2'><font face='Verdana, Arial, Helvetica, sans-serif' size='1' color='#000000'><b>DESCRIÇÃO DETALHADA</b></font></td>
	<td width='5%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'  color='#000000'><b>POS<BR>PED</b></font></td>
...
if (count($array3)<>0){		
	...
	$prod->listProdU("SUM(qtde)","pedido, pedidoprod", "pedidoprod.codprod = $codprodped AND codcb = '' AND pedido.codped = pedidoprod.codped AND cancel <> 'OK' AND pedido.codemp =$codemp  AND pedido.hist <> 1 and dataped < '$dataped'");
	...
	$qtde_estoque = $prod->showcampo0();
			...
			if ($statusprod <> "OK" ){$statusprod = "NO";}
...
");
	...
	$nomeprod</font></td>
	<td width='5%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'  color='#000000'><b>$statusprod</b></font></td>
    <td width='5%' height='0' align='center'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'><b>$qtde</b></font></td>
	<td width='5%' height='0' align='center'>");
for($p = 1; $p <= $qtde; $p++ ){

if (($qtde_estoque - $qtde_ped)-$p >= 0){$status_sep = "DISP";$cor77= '#3366FF';}else{$status_sep = " NDISP";$cor77= '#FF0000';}
...
<div align='center'>
  ...
  <table border='0' width='400' cellspacing='0' cellpadding='3'>
    <tr>
      ...
      <td width='163'>
    <font face='Verdana, Arial, Helvetica, sans-serif' size='1' color='$corcb'><b><a href='javascript:void(0)' onclick=");echo('"');echo("NewWindow('iniciopedadm_est.php?Action=update&desloc=$desloc&codped=$codped&codemp=$codemp&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&retlogin=$retlogin&connectok=$connectok&pg=$pg&impressao=1','name','600','400','yes');return 
...
echo("	
	<td width='93'>
	<font face='Verdana, Arial, Helvetica, sans-serif' size='1' ><b><a href='javascript:void(0)' onclick=");echo('"');echo("NewWindow('iniciocbestoque.php?Action=list&desloc=$desloc&codped=$codped&codbarraped=$codbarra&codemp=$codemp&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&retlogin=$retlogin&connectok=$connectok&pg=$pg&login=$login','name','600','400','yes');return 
false");echo('"');echo(">CONFERÊNCIA DE PRODUTOS</a></b></font>
	</td>
      <td width='72'>
...
size='1'>$datapedf</font></td>
			<td width='15%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif'
size='1' color ='$cor45'>$datapreventf $horaprevent <br><b>$moto</b></font></td>
...
size='1'><b>$status</b><br>$mod</font></td>
			...
			<td align='center' width='5%' height='0'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>");
if ($control_cj > 0){
echo("
		CJ
");
