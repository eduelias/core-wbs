...
switch ($Action) {
	...
	case "inscb":
	 ...
	 $prod->listProdSum("codpped, codprod, codprodcj, tipocj, qtde", "pedidoprod_kit", "codped=$codped", $array3, $array_campo3 , "");
	...
	for($i = 0; $i < count($array3); $i++ ){
		...
		for($j = 0; $j < $qtdeped; $j++ ){
			...
			$codbarra_prod = $codbarra[$codpped][$j];
			#$codbarra_prod = "305HD137798";
			
		//VERIFICA SE EXISTE ALGUM COD BARRA LIVRE
         $reg = $prod->listProd("codbarra", "codbarra = '$codbarra_prod' and codbarraped <> '1' and codprod=$codprod and codemp=$codempselect");
		 
			#echo("$reg - $codprod - $codbarra_prod<br>");
							
			if ($reg <> 0){

				$reg_scb = $prod->listProd("codbarra", "codbarra = '$codbarra_prod' and codbarraped = '1' and codprod=$codprod and codped = $codped and codprodcj = $codprodcj and tipocj=$tipocj and codemp=$codempselect and codped_kit = '$codped'");

				#echo"rcb=$reg_scb - qt=$qtdeped";
			  ...
			  if ($reg_scb == 0){
				...
				// ATUALIZA INFORMAÇÕES DOS PRODUTOS CHECADOS
				
				// INSERE CODBARRA NA TABELA CODBARRA
				$prod->listProd("codbarra", "codbarra='$codbarra_prod' and codbarraped <> 1 and codprod=$codprod  and codemp=$codempselect");
				
				$codcb = $prod->showcampo0();
				$prod->setcampo12($codped); // COD KIT
				...
				$prod->listProd("pedidoprod_kit", "codpped='$codpped'");
				
				#$prod->setcampo4($codped);
				#$prod->setcampo5($codprodcj[$i]);
				#$prod->setcampo6($tipocj[$i]);
				$codcbold = $prod->showcampo10();
				   if ($codcbold == ""){
					$codcbnew = $codcb;
				   }else{
					$codcbnew = $codcbold . ":" . $codcb;
				   }
				$prod->setcampo10($codcbnew);
				$prod->upProd("pedidoprod_kit", "codpped='$codpped'");
		...
		$prod->listProdSum("SUM(qtde) as qtde", "pedidoprod_kit", "codped = $codped", $array613, $array_campo613, "" );
		...
		$prod->listProdSum("COUNT(*) as cbok", "codbarra", "codbarraped = '1' and codped_kit = $codped", $array613, $array_campo613, "" );
		$prod->mostraProd( $array613, $array_campo613, 0 );
		...
		if ($numcbok == $qtde_prodped){
				...
				$codbarra_kit = $prod->showcampo29();
				$cb_ok = $prod->showcampo22();
				
				if($cb_ok == "NO"){

					$prod->setcampo22("OK");  // CODBARRA OK
					$prod->setcampo14("PROD");
					$prod->upProd("pedido_kit", "codped='$codped'");

					// INSERE CODBARRA
					$prod->clear();
					$prod->setcampo1(0);
					$prod->setcampo2($codprodcj);
					$prod->setcampo3($codbarra_kit);
					...
					$prod->setcampo18("KT"); // TIPO PROD
					$prod->setcampo19($codped); // CODKIT
					
					
					$prod->addProd("codbarra", $ureg);

					// ATUALIZA ESTOQUE
					$prod->listProd("estoque", "codemp=$codempselect and codprod=$codprodcj");
					...
					$prod->upProd("estoque", "codemp=$codempselect and codprod=$codprodcj");
				...
				$prod->listProd("pedido_kit", "codped='$codped'");
				$prod->setcampo14("PECA");
				$prod->upProd("pedido_kit", "codped='$codped'");
		
		}

		if ($B1 == "Finalizar"){$Action="list";$Actionsec="list";}
...
if ($Actionsec == "list"){
		...
		$condicao4 = " (pedido_kit.status='APROV' or pedido_kit.status='PECA' or pedido_kit.status='LIB') and";
...
<td width='40%' height='0'  align='center'>
  ...
  <div align='center'>
    ...
    <table border='0' width='550' cellspacing='0' cellpadding='2'>
      <tr>
        <td width='100%' align='center'><input class='sbttn' type='submit' value='Inserir Codbarra' name='B1' onClick=");echo('"MM_changeProp(');echo("'camada','','style.visibility','hidden','LAYER')");echo('"');echo(">&nbsp;&nbsp;&nbsp;&nbsp;<input type='submit' value='Finalizar' name='B1' onClick=");echo('"MM_changeProp(');echo("'camada','','style.visibility','hidden','LAYER')");echo('"');echo("></td>
      </tr>
    </table>
    </center>
  </div>
...
<br>
  ...
  <input type='submit' value='OK' name='B1'>
      ...
      <tr bgColor='#f3f8fa'>
        <td width='186'><font face='Verdana' size='1'><b>RECEBIMENTO PELO ESTOQUE</b></font></td>
        <td width='554'>
...
<table width='550' border='0' cellspacing='1' cellpadding='2' align='center'>
	...
	");
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			if ($status == "LIB"){$contlib=$contlib+1;}



		echo("
<form action='geraetiqueta.php?codped=$codped&retlogin=$retlogin&connectok=$connectok&pg=$pg&codprod=$codprod' method='POST' target='_blank' name='FORM1'>
...
size='1'><b>$status</b><br>$mod</font></td>
		<td align='center' width='5%' height='0'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>$difdias</font></b></td>
			...
			<td align='center' width='10%' height='0'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'><b><input size='2' name='numvol' value='2'><input type='submit' value='GE' name='B1'></font></b></font></b></td>
