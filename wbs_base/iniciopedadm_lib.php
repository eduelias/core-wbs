...
switch ($Action) {
    ...
    case "insert":
		...
		#$prod->listProdgeral("pedidostatus", "codped=$codped and statusped='ENTR'", $array616, $array_campo616 , "");
			...
			$prod->listProdgeral("pedidostatus", "codped=$codped and statusped='L.ENTR'", $array614, $array_campo614 , "");
			...
			if (count($array614) == 0){
				...
				$prod->setcampo28($notafin);
				$prod->setcampo21("$libentrselect"); // LIB. ENTREGA
				$prod->setcampo27("$contratoselect"); // CONTRATO
				$prod->setcampo44("$modpedselect"); // MODPED
				if ($fat == "OK" and ($contratoselect == "OK" or $contratoselect == "EP" or $contratoselect == "DC")){
				$prod->setcampo21('OK');
				}
				...
				// ATUALIZA STATUS DA TABELA
				$prod->listProd("pedidostatus", "codped=$codped and statusped='L.ENTR'");
				$prod->setcampo2($dataatual);
				$prod->setcampo4($login);
				$prod->upProd("pedidostatus", "codped=$codped and statusped='L.ENTR'");
...
if ($Actionsec == "list"){
		...
		$condicao4 = " (pedido.status <>'AVAL' and (pedido.libentr = 'NO' or  pedido.contrato = 'NO')) and ";
		...
		// LISTA OS REGISTROS DE ACORDO COM O ACRESCIMO DEFINIDO
		$prod->listProdSum("codped, pedido.codcliente, pedido.codvend, dataped, dataprevent, status, horaprevent, nf, contrato, libentr, codbarra, caixa, clientenovo.nome, fat, modped", "pedido, clientenovo, vendedor", $condicao3, $array, $array_campo, $parm );
...
");
	// VERIFICA SE O PEDIDO CARTAO VISA 
		...
		if ($parc_visa > 0  or $parc_master > 0 ){$cartao = 1;}else{$cartao=0;}

if ($lib_ped_vend == "Y" and $cartao == 0){ // GRUPO ADMIN
echo("
      <tr bgColor='#f3f8fa'>
        <td width='186'><font face='Verdana' size='1'><b>LIB. ENTREGA</b></font></td>
        <td width='554'>
  <select size='1' class=drpdwn name='libentrselect'>

	<option value='OK'  >OK</option>
    <option value='NO'>NO</option>
	<option value='$libentr' selected>$libentr</option>				  
	...
	echo("
<input type='hidden' name='libentrselect' value='NO'>
	");
}

if ($codgrpselect == 2 OR $codgrpselect == 24){ // GRUPO ADMIN
echo("

	   <tr bgColor='#f3f8fa'>
        <td width='186'><font face='Verdana' size='1'><b>STATUS MOD</b></font></td>
        <td width='554'>
  <select size='1' class=drpdwn name='modpedselect'>

    <option value='NO'>NO</option>
	<option value='$modped' selected>$modped</option>				  
      ...
      <tr bgColor='#f3f8fa'>
        <td width='186'><font face='Verdana' size='1'><b>NOTA FINANCEIRO:</b></font></td>
        <td width='554'>
			<input type='text' name='notafin' value='$notafin' size='48'>
        </td>
      </tr>
");
}else{
	echo("
<input type='hidden' name='modpedselect' value='$modped'>
<input type='hidden' name='notafin' value='$notafin' size='48'>
	");
}
echo("
    </table>
