...
if ($Actionsec == "list"){
		...
		#$condicao5 = " LCASE(vendedor_usuario.login_user) like '%$nomevendpesq1%' and ";
		...
		//PESQUISA POR CODBARRA
			...
			if ($_SESSION['tipo_user'][$pg] == 'N'){
				$condicao3 .= " and pedido.codvend_user='".$_SESSION['codvend_sess_user'][$pg]."'";
			}
			if ($_SESSION['tipo_user'][$pg] == 'S' and $codvend_user_pesq <> '' ){
				$condicao3 .= " and pedido.codvend_user='".$codvend_user_pesq."'";
			}
		...
		$prod->listProdSum("codped, pedido.codcliente, pedido.codvend, dataped, dataprevent, status, horaprevent, nf, contrato, libentr, codbarra, caixa, nome, pendfpg, reavalfpg, fat, modped, dataprevent_old, aguard_comp, inicio_sep, declara, onsite, modelo_ped, prontaentr, codvend_user, vpp, ((mlb-mlb_dist)/vc)*100 as porc", "pedido, clientenovo", $condicao3, $array, $array_campo, $parm );
		}
...
if($Action == "insert" or $Action == "update" or $Action == "delete"):
	...
	// MOSTRA DADOS DO PEDIDO - INICIO
	 $prod->listProd("pedido", "codped=$codped");
		...
		$mlb = $prod->showcampo7();
		$mlb_dist = $prod->showcampo95();
		$mcv = $prod->showcampo19();
		$vpv = $prod->showcampo8();
		$vt = $prod->showcampo9();
		$vpp = $prod->showcampo10();
		$mlbf = number_format(($mlb-$mlb_dist),2,',','.'); 
		...
		$codvend_user_ped = $prod->showcampo98();
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td width='51' bgcolor='#FFFFFF'>
                          <p align='center'><font size='1' face='Verdana'><a href='$PHP_SELF?Action=list&amp;codempselect=$codemp&amp;codped=$codped&amp;desloc=$desloc&amp;palavrachave=$palavrachave&amp;operador=$operador&amp;pagina=$pagina&amp;retlogin=$retlogin&amp;connectok=$connectok&amp;pg=$pg&amp;hist=$hist&codvend_user_pesq=$codvend_user_pesq&mesped=$mesped&anoped=$anoped&hist_todos=$hist_todos'><img border='0' src='images/bt-continuaped.gif' ><br>
...
<div align='center'>
  <table border='0' width='550' cellspacing='1' cellpadding='2'>
    ...
    <tr>
      ...
      </font><font color='#FFFFFF' face='Verdana, Arial, Helvetica, sans-serif'>$nomevendold ($nome_vend_user)</font></b></font></td>
      </tr>
...
");
		if ($modelo_ped == "ATA" or $modelo_ped == "DST" or $modelo_ped == "RDST" or $modelo_ped == "RDD"){$impressao = 1;}
if ($impressao <> 1){
echo("
...
if ($modelo_ped == "ATA" or $modelo_ped == "DST" or $modelo_ped == "RDST" or $modelo_ped == "RDD"){$impressao =0;}
if ($impressao <> 1 ){
echo("
 ...
 <div align='center'>
	...
	");
	if ($_SESSION['tipo_user'][$pg] == 'S' and $modelo_ped == 'RDD' ){
	echo("
	<tr bgcolor='#ffffff'>
        <td width='27%'><font size='1' face='Verdana' color='#336699'><b>MLB:</b></font></td>
		<td width='39%' ><font size='1' face='Verdana'><b>R$ $mlbf</b></font></td>
        <td width='15%'><font size='1' face='Verdana' color='#666666'><b></b></font></td>
        <td width='19%' ><font color='#666666' size='1' face='Verdana'><b> </b></font></td>
	</tr>
	<tr bgcolor='#ffffff'>
...
<font color='#336699' face=' Verdana, Arial, Helvetica, sans-serif' size='2'>$subtitulo</font></td>
    ...
    <option selected value='oc'>Ordem Compra</option>
		...
		CLIENTE: <input type='text' name='palavrachave' size='20'><br>
			VENDEDOR: <select name='codvend_user_pesq' class='cmp1'>
 ...
 ");
  ...
  echo ("
          ...
          <option value='' selected>ESCOLHA</option>
                </select><br>PERÍODO: <input type='text' name='mesped' size='2' maxlength = '2'>/<input type='text' name='anoped' size='4' maxlength = '4'><input type='checkbox' name='hist_todos' value ='OK' >TODOS PEDIDOS</font>
  ...
  <tr>
	...
	");
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			$codvend_user_ped = $prod->showcampo24();
			$vpp = $prod->showcampo25();
			$porc_mlb = $prod->showcampo26();
			$vppf = number_format($vpp,2,',','.'); 
			$porc_mlbf = number_format($porc_mlb,2,',','.'); 
...
size='1'><b><a
href='$PHP_SELF?Action=update&codped=$codped&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&retlogin=$retlogin&connectok=$connectok&pg=$pg&hist=$hist&codvend_user_pesq=$codvend_user_pesq&mesped=$mesped&anoped=$anoped&hist_todos=$hist_todos'>$codbarra</font></b></a><br><font face='Verdana, Arial, Helvetica, sans-serif'
...
size='1' color='#FF0000'><b>$nome_vend_user</b></font></font><BR><font face='Verdana, Arial, Helvetica, sans-serif' size='1' color = '$cor7'>$ped_ata</font></td>
			<td width='15%' height='0' align='center'><font face='Verdana, Arial, Helvetica, sans-serif'
size='1'><b>R$$vppf</b>");
	if ($_SESSION['tipo_user'][$pg] == 'S' and $modelo_ped <> "RDST" ){
	echo("<br><font face='Verdana, Arial, Helvetica, sans-serif'
size='1' color='#FF0000'>($porc_mlbf%)</font>");
	}
	echo("<BR>$datapedf</font></td>
