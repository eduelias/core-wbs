    ...
    case "insert":
	 ...
	 case "insert_aval": 
		...
		$dataatual = $prod->gdata();
			...
			// ATUALIZA BANCO DE DADOS DE PEDIDOS
			if ($tipocliente_aval == "F"){$cond_aval = " cpf = '$cpf_aval' ";}else{$cond_aval = " cnpj = '$cnpj_aval' ";}
			$prod->clear();
			$prod->listProdU("codcliente", "clientenovo", $cond_aval." and avalista = 'S' ");
			$codcliente_aval= $prod->showcampo0();

			if($codcliente_aval){
			$prod->upProdU("pedido","avalista = '$codcliente_aval'", "codped='$codped'");
			...
			$prod->setcampo1($codped);
			$prod->setcampo2($dataatual);
			$prod->setcampo3("ADD AVALISTA");
			$prod->setcampo4($login);
	
			$prod->addProd("pedidostatus", $ureg);
			$erro_avalista = "";
			}else{
			$erro_avalista = "CLIENTE NÃO CLASSIFICADO COMO AVALISTA";
			}
			$Action= "update";
		...
		*/
		...
		if ($valorvendavista > $valorminimovenda){
			...
			*/
			...
			#if($contrato==""){$contrato = $prod->showcampo27();}
  ...
  for (o = oini; o < (oini+10); o++)
  {
   ...
   strNum = oini + 8;
	...
	prazo_medio_parc = daysparc * parseInt(form.elements[strValor].value);
     ...
     //alert ('VALOR ' + dateped + dateparc +' - '+ daysparc +' - '+ prazo_medio_parc +' INFERIOR a data do pedido.');
	...
	if (form.elements[strTipo].value != 0)
	{
 	...
 	prazo_medio = prazo_medio_total/valor_parc;
	  //alert('As PARCELAS NAO PODERAO SER ALTERADAS! O prazo medio(' + prazo_medio_total + ' - ' +  valor_parc + ' - '  + prazo_medio + ')  das parcelas e superior a 150 dias. ');
...
echo("
 ...
 ");
 if ($modelo_ped == "DST" and $checar_aval == 1){
 echo (" 
...
<table width='550'  border='0' cellspacing='0' cellpadding='0'>
  <tr>
    <td><fieldset class='pesq_cad_field'>
      <legend class='pesq_cad_legend'>INSERIR AVALISTA: </legend>
      <table  border='0' cellspacing='2' cellpadding='2'>
        <form action='$PHP_SELF?Action=insert_aval' method='POST' name='form99'>
          <tr>
          <td valign='top'><table  border='0' cellspacing='2' cellpadding='2'>
                <tr>
                  <td align='center' valign='middle'><input name='tipocliente_aval' type='radio' value='F' checked></td>
                  <td>CPF<br>
                      <input id='cpf_aval' name='cpf_aval' type='text' class='campo3' size='16' maxlength='14' onKeyUp=\"mascara_cpf(this.form, 'cpf_aval', this.value)\"></td>
                </tr>
                <tr>
                  <td align='center' valign='middle'><input name='tipocliente_aval' type='radio' value='J'></td>
                  <td>CNPJ<br>
                      <input id='cnpj_aval' name='cnpj_aval' type='text' class='campo3' size='21' maxlength='18' onKeyUp=\"mascara_cnpj(this.form, 'cnpj_aval', this.value)\"></td>
                </tr>
            </table></td>
            <td valign='middle'>
            <input name='Submit' type='submit' class='botao3' value='Pesquisar'></td>
          </tr>
		  	<input type='hidden' name='desloc' value='0'>
		<input type='hidden' name='operador' value='OR'>
        <input type='hidden' name='palavrachave' value='$palavrachave'>
	  ...
	  echo ("
	  <center><b><font face='Verdana, Arial, Helvetica, sans-serif' size='2' color ='#FF0000'>$erro_avalista</font></b><center>
      <table width='550' border='0' cellspacing='0' cellpadding='0'>
        <tr>
          <td bgcolor='#A7FC98'><strong>NOME AVALISTA</strong></td>
          <td bgcolor='#A7FC98'><strong>CPF/CNPJ</strong></td>
        </tr>
        <tr>
          <td>$nome_aval</td>
          <td>$doc_aval</td>
...
if ($tipovend <> 'R' and $var_codgrp <> 21 and $var_codgrp <> 62 and $var_codgrp <> 42 and $var_codgrp <> 13 and $var_codgrp <> 56 and $var_codgrp <> 54 and $var_codgrp <> 32){
	...
	if ($tipovend <> 'R'){
		...
		if (($codemp == 14 and ($var_codgrp == 66 or $var_codgrp == 58) ) or $codemp == 15 or $codemp == 20 ){
		echo("		
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=$codemp&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&nomevendpesq=$nomevendpesq&diaspg=$diaspg#cliente'>$nomeemp</option>
		");
		}
...
if ($ped_transf == "OK"){$cor8 ="#008000";$ped_transf = "TRANSF";}else{$ped_transf = "";}
		...
		#echo "codped = $parc_visa-$cartao, $parc_master-$cartao1<br>";
