...
$nomeform = "PEDIDO";
$titulo = "ADMINISTRA��O SUPERVISOR";
$subtitulo = "PEDIDOS";
		...
		*/
		...
		if ($valorvendavista > $valorminimovenda){
			...
			*/
			...
			// INSERE PARCELAS NO BANCO DE DADOS
			 ...
			 for($i = 0; $i < count($array612); $i++ ){
				...
				/*
			// VERIFICA SE A FORMA DE PAGAMENTO AGUARDA COMPRESA��O
			$prod->clear();
			...
			*/
			...
			$Actionter = "lock";
			$prod->msggeral("O pedido n�o foi aceito !", "ERRO", "$PHP_SELF?Action=list&desloc=$desloc&codpedselect=$codpedselect&retlogin=$retlogin&connectok=$connectok&pg=$pg&pgold=$pgold", 0);
		}
...
if ($Action_mod == "end"){
			...
			#$prod->setcampo44("OK");  // MODIFICA��O - PEDIDO
			if ($control == "S"){
			...
			$prod->listProdU("mlb, vc, fatorcomissao, tipo, pedido.codvend, codbarra, codvend_cm, porc_cm", "pedido, vendedor", "codped=$codped and pedido.codvend=vendedor.codvend");
					...
					$prod->setcampo3("Estorno Comiss�o Pedido");
			...
			#$Actionter = "lock";
			#$prod->msggeral("Modifica��o efetuada com sucesso !", "OK", "$PHP_SELF?Action=list&desloc=$desloc&codpedselect=$codpedselect&retlogin=$retlogin&connectok=$connectok&pg=$pgtroca&pgold=$pgold", 0);
 ...
 for (cont = 1; cont <= qtde; cont++)
 {
  ...
  for (o = oini; o < (oini+10); o++)
  {
   ...
   strNum = oini + 8;
	...
	{
		alert ('Valor da Parcela ' + cont + ' : preenchimento obrigat�rio.');
		eval ('form.elements[strValor].focus ()');
		return false;
	}
	if (verificaNumerico (form.elements[strValor].value, 2) != 1)
	{
		alert ('Valor da Parcela ' + cont + ' formato inv�lido');
	...
	if (form.elements[strTipo].value != 0)
	{
		...
		if ((form.elements[strTipo].value == '02.01') && ((form.elements[strBanco].value == 0) || (form.elements[strAgencia].value == 0) || (form.elements[strNum].value == 0) || (form.elements[strConta].value == 0)))
		{
			...
			alert ('Banco, Agencia, Conta e Num Cheque da Parcela ' + cont + ' : preenchimento obrigat�rio.');
		...
		*/
		...
		alert ('Forma de Pagamento' + cont + ' : preenchimento obrigat�rio.');
		eval ('form.elements[strTipo].focus ()');
		return false;
	 ...
	 //alert ('prazo_medio_total' + prazo_medio_total + ' : preenchimento obrigat�rio.');
    ...
    {
        alert('IMPORTANTE! Voc� possui um COMPUTADOR PARA TODOS no seu carrinho! Esse produto so poder� ser vendido atrav�s do FINANCIAMENTO CEF para todos ou FINANCIAMENTO BCO BRASIL para todos. ');
...
function ControleImbecil() 
{
	...
	alert ('O contrato n�o poder� ser impresso, pois algum cheque n�o foi preenchido');
...
function Envia(tipo)
{
    if (duplaSubmissao > 0)
    {
        alert('Por favor, aguarde. Proposta j� enviada ao sistema.');
        return false;
    }
	...
	if (days <= -1){
		alert('DATA DE PREVIS�O DE ENTREGA inferior a DATA ORIGINAL DE ENTREGA');
...
<div align='center'>
  <center>
    ...
    <tr>
      ...
      <td width='304' bgcolor='#F0F0F0' valign='top' align='right'><font color='#336699' face='Verdana, Arial, Helvetica, sans-serif' size='1'><b>PREVIS�O
        ENTREGA:<br>
        </b></font><font face='Verdana, Arial, Helvetica, sans-serif' color='#000000' size='1'>$datapreventf - $horaprevent</font><BR>
		 <font color='#336699' face='Verdana, Arial, Helvetica, sans-serif' size='1'><b>PREVIS�O
        ENTREGA ORIGINAL:<br>
...
<form method='POST' action='$PHP_SELF?Action=insert'  name='Form1'>
 <div align='center'>
	...
	<tr bgColor='#f3f8fa'>
        <td width='186'><font face='Verdana' size='1'><b>NOVA DATA DE PREVIS�O DE ENTREGA:</b></font></td>
        <td width='554'>
...
<table width='550' border='0' cellspacing= '0' cellpadding='0' align='center'>
  <tr>
    <td><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#336699'>PARTE 
      III</font><font size='1' face='Verdana, Arial, Helvetica, sans-serif'> - ALTERAR CONDI��ES DE PAGAMENTO</font></b></td>
  </tr>
</table>
		");
...
echo("   
    ...
    <table border='0' width='550' cellspacing='0' cellpadding='2'>
      <tr>
        <td width='100%' align='center'><input class='sbttn' type='submit' value='Alterar Condi��es de Pagamento' name='B1'></td>
      </tr>
    </table>
    </center>
  </div>
...
<form action='$PHP_SELF' name='form79' method='post' >
  <tbody>
    ...
    <tr bgColor='#FCD7C5'>
      <td colspan='3'><font color='#FF0000' size='1' face='Verdana, Arial, Helvetica, sans-serif'><strong>ATEN&Ccedil;&Atilde;O:</strong> </font><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>O MICRO ou as PE&Ccedil;AS AVULSAS s&oacute; come&ccedil;ar&atilde;o a ser separadas quando a OP��O abaixo for selecionada. </font></td>
    </tr><!--
    <tr bgcolor='#FCD7C5'>
...
echo("
 ...
 <div align='center'>
	...
	<tr bgcolor='#ffffff'>
		...
		<tr bgcolor='#ffffff'>
        <td width='27%'><font size='1' face='Verdana' color='#336699'><b>VALOR � VISTA:</b></font></td>
...
<FONT >
	...
	$prod->listProdSum("codemp, nome", "empresa", " hist_emp <> 'S' ", $array6, $array_campo6 , "order by nome");
...
if ($tipovend <> 'R' and $var_codgrp <> 21 and $var_codgrp <> 62 and $var_codgrp <> 42 and $var_codgrp <> 13 and $var_codgrp <> 56 and $var_codgrp <> 54 and $var_codgrp <> 32){
	...
	if ($tipovend <> 'R'){
		...
		if ($codemp == 1 or $codemp == 15 or $codemp == 20){
		echo("		
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=$codemp&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&nomevendpesq=$nomevendpesq&diaspg=$diaspg#cliente'>$nomeemp</option>
		");
		}
...
if ($aguard_comp == "OK"){$aguard ="AGUARDA<br>COMPENSA��O";}else{$aguard ="";}
#if ($declara == "EP" or $declara == "NO" ){$declara_info ="AGUARDA<br>DECLARA��O";}else{$declara_info ="";}
if ($contrato == "NO"){$cor1 ="#FF0000";}else{$cor1="#008000";}
if ($contrato == "EP"){$cor1 ="#FF6600";}
if ($libentr == "NO"){$cor2 ="#FF0000";}else{$cor2="#008000";}
if ($fat == "NO"){$cor3 ="#FF0000";}else{$cor3="#008000";}
  ...
  <table border='0' width='550' cellspacing='3' cellpadding='3'>
    <tr>
      ...
      <td width='11%' align='center' bgcolor='#BFD9F9'><font size='1' face='Verdana' class='texto'><b>PEDIDO
        ENTREGUE</b></font></td>
      <td width='11%' align='center'><font size='1' face='Verdana' class='texto'><b>CR�DITO
        REPROVADO</b></font></td>
