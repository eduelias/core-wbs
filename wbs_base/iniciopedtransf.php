...
$prod = new operacao();
	...
	$prod->listProd("seguranca", "arquivo='iniciopedtransffpg.php'");
	...
	#$prod->listProd("seguranca", "arquivo='inicioclienteform.php'");
		...
		// CODEMP DESTINO TRANSFERENCIA
		$prod->clear();
        $prod->listProdU("codemp_transf, codgrp","vendedor", "nome='$login'");
		$codemp_transf = $prod->showcampo0();
		$codgrp_transf = $prod->showcampo1();
		$codemptransfselect = $codemp_transf;
        ...
        $prod->clear();
        $prod->listProdU("nome, allemp, fatorusertabela, codemp_transf, tipocliente, doc","vendedor", "codvend='$codvend_transf'");
	...
	");
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			if ($puc_prod == 0){
				// VERIFICA ULTIMO PRECO DE COMPRA
				$prod->clear();
				$prod->listProdU("pcu, icms, ipi", "ocprod, oc ", "codprod = $codprod[$i] and ocprod.codemp=$codempselect and hist = 1 and oc.codoc=ocprod.codoc  ORDER BY oc.datacompra DESC LIMIT 0,1");
				$pcu_oc = $prod->showcampo0();
				$ppu  = $pcu_oc;
		...
		echo("
		 <tr bgcolor='$cor'> 
                ...
                <td width='10%' align='center' ><font face='Verdana' size='1'> 
                 ");if (($estoque>0 and $ppu > 0) or $codgrp_transf == 53 or $codgrp_transf == 34){ echo("<input type='text' name='qtde[$i]' size='4' maxlength='3' value='0'>");}echo("

	</font></td>
                <td width='10%' align='center' ><font face='Verdana' size='1'><strong>$ppuf</strong></font></td>
                <td width='20%' align = 'center' > 
                 ");if (($estoque>0 and $ppu > 0) or $codgrp_transf == 53 or $codgrp_transf == 34){ echo("
	");
	#if ($unidade[$i] <> "CJ"){
		echo("
...
if ($codgrp_vend == 2 or $codgrp_vend == 34 ){

if ($allemp == "Y" and $block_transf <> 1){
...
if ($codemp_transf_loja <> 0 and $block_transf_loja <> 1){
echo("
<br><br>
<table width='550' border='0' cellspacing= '0' cellpadding='0' align='center'>
  <tr>
	...
	$prod->listProdU("nome","empresa", "codemp=1");
	$nomeemp_padrao = $prod->showcampo0();

	$prod->listProdU("nome","empresa", "codemp=$codemp_transf_loja");
	$nomeemp_transf_loja = $prod->showcampo0();

	echo("
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=1&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>$nomeemp_padrao</option>
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=$codemp_transf_loja&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>$nomeemp_transf_loja</option>
...
");
  ...
  if (!$codempselect){$codempselect = 1;}
...
if ($codgrp_vend <> 64){
echo("	
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=13&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>IPASOFT BH SHOPPING</option>
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=10&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>IPASOFT HALFELD</option>
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=2&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>IPASOFT INDEPENDÊNCIA</option>
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=12&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>IPASOFT MINAS SHOPPING</option>
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=1&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>IPASOFT PRODUÇÃO</option>
");
}else{
	echo("
		<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=14&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>MAXXMICRO</option>
			<option value='$PHP_SELF?Action=list&codprod=$codprod&codempselect=15&codemptransfselect=$codemptransfselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&codpedimport=$codpedimport&orcimp=$orcimp#cliente'>MAXXMICRO LOJA FABRICA</option>
