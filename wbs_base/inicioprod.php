...
switch ($Action) {
    ...
    case "insert":
		...
		$descdet = htmlentities($descdet, ENT_QUOTES);
		...
		$prod->setcampo30($dataatual); // DATA CADASTRO
		$prod->setcampo31($dataatual); // DATA ATUALIZACAO
		$prod->setcampo32($cor1);
		$prod->setcampo33($cor2);
		$prod->setcampo34($garext_12);
		$prod->setcampo35($garext_24);
		$prod->setcampo36($porc_garext_12);
		$prod->setcampo37($porc_garext_24);
		$prod->setcampo38($ponto_prod);
		if ($codmarca == ""){$codmarca = 1;}
		$prod->setcampo39($codmarca);
		$prod->setcampo40($dataatual); // DATA ATUALIZACAO PRECO
		$prod->setcampo41($garonsite_12);
		$prod->setcampo42($garonsite_24);
		$prod->setcampo43($v_garonsite_12);
		$prod->setcampo44($v_garonsite_24);
		$prod->setcampo45($opcaixa10x);
		$prod->setcampo46($chk_ppb);
		$prod->setcampo47($chk_icms);
		$prod->setcampo48($chk_pis_confins);
		$prod->setcampo49($lib_contrato);
		$pvd = str_replace('.','',$pvd);
		$pvd = str_replace(',','.',$pvd);
		$prod->setcampo50($pvd);
		$pvdcj = str_replace('.','',$pvdcj);
		$pvdcj = str_replace(',','.',$pvdcj);
		$prod->setcampo51($pvdcj);
		if ($fdist < 0.9){$fdist = 0.98;}
		$prod->setcampo52($fdist);
		$prod->setcampo53($tipo_gar);
		$prod->setcampo54($chk_web);
		$prod->setcampo55($chk_st);
		$prod->setcampo56($ncm);
		$aliquota_icms_st = str_replace('.','',$aliquota_icms_st);
		$aliquota_icms_st = str_replace(',','.',$aliquota_icms_st);
		$prod->setcampo57($aliquota_icms_st);
    ...
    case "update":
		...
		$datacad = $prod->fdata($prod->showcampo30());
		$dataatualiza = $prod->fdata($prod->showcampo31());
		
		
		//DADOS DE COR
		$cor1old = $prod->showcampo32();
		$cor2old = $prod->showcampo33();
		
		//GARANTIA EXTENDIDA
		$xgarext_12 = $prod->showcampo34();
		$xgarext_24 = $prod->showcampo35();
        $xporc_garext_12 = $prod->showcampo36();
		$xporc_garext_24 = $prod->showcampo37();
        $xponto_prod = $prod->showcampo38();
		$xcodmarca = $prod->showcampo39();
		$xdataatual_preco = $prod->fdata($prod->showcampo40());
		
		//GARENTIA ONSITE
		$xgaronsite_12 = $prod->showcampo41();
		$xgaronsite_24 = $prod->showcampo42();
		$xv_garonsite_12 = $prod->showcampo43();
		$xv_garonsite_24 = $prod->showcampo44();
		$xopcaixa10x = $prod->showcampo45();
		$chk_ppbx = $prod->showcampo46();
		$chk_icmsx = $prod->showcampo47();
		$chk_pis_confinsx = $prod->showcampo48();
		$lib_contratox = $prod->showcampo49();
		$pvdold = $prod->showcampo50();
		$pvdold = number_format($pvdold, 2,',', '.');
		$pvdcjold = $prod->showcampo51();
		$pvdcjold = number_format($pvdcjold, 2,',', '.');
		$fdistx = $prod->showcampo52();
		$tipo_garx = $prod->showcampo53();
		$chk_webx = $prod->showcampo54();
		$chk_stx = $prod->showcampo55();
		$ncmx = $prod->showcampo56();
		$aliquota_icms_stx = $prod->showcampo57();
		$aliquota_icms_stx = number_format($aliquota_icms_stx, 2,',', '.');
	 ...
	 case "delete":
 	 ...
 	 $prod->delProd("produtos_planos_relacao", "codprod = '$codprod'");
 	 
 	 $prod->listProdSum("codplano" , "produtos_planos", "hist <> 'S'", $array45, $array_campo45, "order by plano" );

	 for($i = 0; $i < count($array45); $i++ ){

			$prod->mostraProd( $array45, $array_campo45, $i );

			$codplano = $prod->showcampo0();
			if ($codplano == $codplano_sel[$i]){

                $desconto_sel[$i] = str_replace('.','',$desconto_sel[$i]);
    			$desconto_sel[$i] = str_replace(',','.',$desconto_sel[$i]);
              
                $prod->clear();
              	$prod->setcampo0($codprod);
				$prod->setcampo1($codplano);
				$prod->setcampo2($desconto_sel[$i]);
				$prod->setcampo3("S");
                $prod->setcampo4($comissao_sel[$i]);

                $prod->addProd("produtos_planos_relacao", $ureg1);
...
if ($Actionsec == "list"){
		...
		$prod->listProdSum("codprod, nomeprod, pvv, pva, unidade, hist, kit, pvd , descres", $tabela, $condicao2, $array, $array_campo, $parm );
...
/// INICIO - PARTE DE UP/ADD DOS REGISTROS ////
	...
	<script type=\"text/javascript\">
      ...
      {
        var oFCKeditor = new FCKeditor( 'descdet' ) ;
						...
						");
						  ...
						  echo("
                                      ...
                                      <td width='50%'><b><font face='Verdana' size='2'>COD.:
                                        ...
                                        <b>tipo: </b>$imgtypeold<br>
										<b>data atualiza: </b>$dataatualiza<br>
										<b>data cadastro: </b>$datacad</font></td>
                                    </tr>
                                  </table>
...
<tr> 
			...
			echo("
						  <tr bgcolor='#D6E7EF'> 
                            ...
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
                              <!--<input type='text' value='$descresold' name='descres' size='30' maxlength='250'>-->
							  <textarea name='descres'  cols='50' rows='5' onKeyDown=\"if(this.value.length >= 250){this.value = this.value.substring(0, 250)}\" onKeyUp=\"if(this.value.length >= 250){this.value = this.value.substring(0, 250)}\">$descresold</textarea>
						   ...
						   <tr bgcolor='#D6E7EF'> 
                            ...
                            <td width='100%' colspan='2'><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
                              ...
                              <textarea id='descdet' name='descdet'>$descdetold</textarea>
                              </font>
                              <!--
			...
			<tr bgcolor='#D6E7EF'> 
                            <td width='26%' ><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Marca</font></b></td>
                            <td width='74%' colspan ='2'><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
                             <select size='1' class=drpdwn name='codmarca'>
                             
						  
	");

	$prod->listProdgeral("produtos_marcas", "hist <> 'S'", $array, $array_campo , "order by marca");
	for($i = 0; $i < count($array); $i++ ){
			
			$prod->mostraProd( $array, $array_campo, $i );

			$marca = $prod->showcampo1();
			$codmarca = $prod->showcampo0();
	echo("		
	...
	if ($Action=="update"){
		...
		$prod->listProdgeral("produtos_marcas", "codmarca=$xcodmarca", $array, $array_campo , "");
		$prod->mostraProd( $array, $array_campo, 0 );
			$marca = $prod->showcampo1();
		echo("		
		<option selected value='$xcodmarca'>$marca</option>
		");
	}
  ...
  <tr bgcolor='#D6E7EF'>
    <td width='26%' bgcolor='#D6E7EF'><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'><b>Cor Primária:</b></font></td>
    <td width='74%' bgcolor='#D6E7EF'><select name='cor1' style='color: black; font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 11px;'>
      <option value='' selected>Nenhuma</option>
      <option value='#FFFF00' style='color: black;background-color: #FFFF00;'>Amarela</option>
	  <option value='#0000FF' style='color: black;background-color: #0000FF;'>Azul</option>
	  <option value='#FFFFFF' style='color: black;background-color: #FFFFFF;'>Branco</option>
	  <option value='#FFCC00' style='color: black;background-color: #FFCC00;'>Ouro</option>
	  <option value='#CCCCCC' style='color: black;background-color: #CCCCCC;'>Prata</option>
	  <option value='#000000' style='color: white;background-color: #000000;'>Preto</option>
	  <option value='#FF00FF' style='color: black;background-color: #FF00FF;'>Rosa</option>
	  <option value='#009933' style='color: black;background-color: #009933;'>Verde</option>
	  <option value='#FF0000' style='color: black;background-color: #FF0000;'>Vermelha</option>
");
  	if ($Action=="update"){
echo("

  <option value='$cor1old' style='color: black;background-color: $cor1old;' selected>Atual</option>
");

  	} 
echo("

    </select><table  border='0' cellpadding='0' cellspacing='0' bgcolor='$cor1old'>
      <tr>
        <td style='border: 1px solid #000000'><img src='wbs2/images/fcor.gif' width='10' height='10'></td>
      </tr>
    </table></td>
  </tr>
  <tr bgcolor='#D6E7EF'>
    <td width='26%' bgcolor='#D6E7EF'><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'><b>Cor Secund&aacute;ria:</b></font></td>
    <td width='74%' bgcolor='#D6E7EF'><select name='cor2' style='color: black; font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 11px;'>
      <option value='' selected>Nenhuma</option>
      <option value='#FFFF00' style='color: black;background-color: #FFFF00;'>Amarela</option>
	  <option value='#0000FF' style='color: black;background-color: #0000FF;'>Azul</option>
	  <option value='#FFFFFF' style='color: black;background-color: #FFFFFF;'>Branco</option>
...
echo("
  ...
  <option value='$cor2old' style='color: black;background-color: $cor2old;' selected>Atual</option>
");

  	} 
echo("
    </select><table  border='0' cellpadding='0' cellspacing='0' bgcolor='$cor2old'>
      <tr>
        <td style='border: 1px solid #000000'><img src='wbs2/images/fcor.gif' width='10' height='10'></td>
      </tr>
    </table></td>
...
if (($Action == "insert" and $unidade <> "CJ") or ($Action == "update" and $unidadeold <> "CJ")){
	echo("
                            ...
                            <tr bgcolor='#D6E7EF'> 
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>PUC:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
						  ...
						  <tr bgcolor='#D6E7EF'> 
						   ...
						   <tr bgcolor='#D6E7EF'> 
                            ...
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
                              ...
                              </font> <b>Atualizado em:</B> $xdataatual_preco</td>
                          </tr>
						  <tr bgcolor='#D6E7EF''> 
						   ...
						   <tr bgcolor='#D6E7EF'> 
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Preco Venda Distribuição:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
                              <input type='text' value='$pvdold' name='pvd' size='10' onChange='consisteValor(this.form, this.form.pvd.name, true)'>
                              </font></td>
                          </tr>
						   <tr bgcolor='#D6E7EF'> 
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Preco Venda Distribuição CJ:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
                              <input type='text' value='$pvdcjold' name='pvdcj' size='10' onChange='consisteValor(this.form, this.form.pvdcj.name, true)'>
                              </font></td>
                          </tr>
						    <tr bgcolor='#D6E7EF'> 
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Fator Distribuição </font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'> 
                              <input type='text' value='$fdistx' name='fdist' size='8' >
                              </font></td>
                          </tr>
...
");
			if ($Action == "update"){
				if ($hist_prodold=="N" or $hist_prodold==""):
				echo("
								 <input type='checkbox' name='hist_prod' value='Y'  > Produto nao será listado em futuros pedidos por estar fora de linha.
					");
				else:
				echo("
								 <input type='checkbox' name='hist_prod' value='Y' checked> Produto nao será listado em futuros pedidos por estar fora de linha.
					");
				endif;
			}else{
				echo("
...
");
			...
			echo("
                            ...
                            <tr bgcolor='#D6E7EF'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Ponto Produto:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                              <input type='text' value='$xponto_prod' name='ponto_prod' size='10' onChange='consisteValor(this.form, this.form.pvacj.name, true)'>
                              </font></td>
                          </tr>
                          
                           <tr bgcolor='#D6E7EF'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Formapg 10X:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                              <input type='text' value='$xopcaixa10x' name='opcaixa10x' size='40' >
                              </font></td>
                          </tr>
                          
                          <tr bgcolor='#D6E7EF'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>PPB:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                             <select name='chk_ppb'>
					           <option value='S' selected>S</option>
					           <option value='N'>N</option>
					           <option value='$chk_ppbx' selected>$chk_ppbx</option>
					         </select>
                              </font></td>
                          </tr>
						   <tr bgcolor='#FFCC99'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>ST:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                             <select name='chk_st'>
					           <option value='S' selected>S</option>
					           <option value='N'>N</option>
					           <option value='$chk_stx' selected>$chk_stx</option>
					         </select>
                              </font></td>
                          </tr>
						  
						   <tr bgcolor='#FFCC99'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>ALIQUOTA ICMS/ST:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                              <input type='text' value='$aliquota_icms_stx' name='aliquota_icms_st' size='6' onChange='consisteValor(this.form, this.form.aliquota_icms_st.name, true)'>
                              </font></td>
                          </tr> 
						   <tr bgcolor='#A4FFA4'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>NCM/SH:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                              <input type='text' value='$ncmx' name='ncm' size='8' maxlength = '8'>
                              </font></td>
                          </tr>
                          ...
                          <tr bgcolor='#D6E7EF'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Diferimento de ICMS:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                                <select name='chk_icms'>
					           <option value='S' selected>S</option>
					           <option value='N'>N</option>
					           <option value='$chk_icmsx' selected>$chk_icmsx</option>
					         </select>
                              </font></td>
                          </tr>
                          <tr bgcolor='#D6E7EF'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Isento PIS + CONFINS:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                                <select name='chk_pis_confins'>
					           <option value='S' selected>S</option>
					           <option value='N'>N</option>
					           <option value='$chk_pis_confinsx' selected>$chk_pis_confinsx</option>
					         </select>
                              </font></td>
                          </tr>
 							<tr bgcolor='#D6E7EF'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Libera Contrato:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                                <select name='lib_contrato'>
					           <option value='S' selected>S</option>
					           <option value='N'>N</option>
					           <option value='$lib_contratox' selected>$lib_contratox</option>
					         </select>
                              </font></td>
                          </tr>
						  	<tr bgcolor='#66CC66'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Internet:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                                <select name='chk_web'>
					           <option value='S' selected>S</option>
					           <option value='N'>N</option>
					           <option value='$chk_webx' selected>$chk_webx</option>
					         </select>
                              </font></td>
                          </tr>
						  <tr bgcolor='#FFFF33'>
                            <td width='26%'><b><font size='1' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Tipo Garantia:</font></b></td>
                            <td width='74%' ><font color='#000000' face='Verdana, Arial, Helvetica, sans-serif' size='1'>
                                <select name='tipo_gar'>
					           ...
					           <option value='$tipo_garx' selected>$tipo_garx</option>
					         </select>
                              </font><li><b>TIPO A</b> - Troca na Loja</li><li><b>TIPO B</b> - Troca 48hs ou encaminha AT Ipasoft</li><li><b>TIPO C</b> - Troca 48hs ou encaminha AT Fabricante</li><li><b>TIPO D</b> - Garantia AT Ipasoft</li><li><b>TIPO E</b> - Garantia AT Fabricante</li><li><b>TIPO X</b> - NÃO CLASSIFICADO</li></td>
                          </tr>
...
");
if ($xgarext_12 == 'OK'){$ck1= "checked";}
if ($xgarext_24 == 'OK'){$ck2= "checked";}
echo("

  <table width='60%' border='1' cellpadding='1' cellspacing='1'>
  <tr bgcolor='#FFFF99'>
    <td colspan='3'><strong>EXTENS&Atilde;O DA GARANTIA </strong></td>
  </tr>
  <tr>
    <td width='39'><input name='garext_12' type='checkbox' id='gar_ext12' value='OK' $ck1></td>
    <td width='117'>12 MESES</td>
    <td width='376'><input name='porc_garext_12' type='text' id='porc_garext12' size='5' maxlength='5' value='$xporc_garext_12'>
      %</td>
  </tr>
  <tr>
    <td><input name='garext_24' type='checkbox' id='gar_ext24' value='OK' $ck2></td>
    <td>24 MESES</td>
    <td><input name='porc_garext_24' type='text' id='porc_garext24' size='5' maxlength='5' value='$xporc_garext_24'>
    %</td>
  </tr>
</table>

");
if ($xgaronsite_12 == 'OK'){$ckos1= "checked";}
if ($xgaronsite_24 == 'OK'){$ckos2= "checked";}
echo("
<br>
  <table width='60%' border='1' cellpadding='1' cellspacing='1'>
  <tr bgcolor='#FFFF99'>
    <td colspan='3'><strong>GARANTIA ON SITE </strong></td>
  </tr>
  <tr>
    <td width='39'><input name='garonsite_12' type='checkbox' id='garonsite_12' value='OK' $ckos1></td>
    <td width='117'>12 MESES</td>
    <td width='376'>R$ <input name='v_garonsite_12' type='text' id='v_garonsite_12' size='5' maxlength='5' value='$xv_garonsite_12'>
      </td>
  </tr>
  <tr>
    <td><input name='garonsite_24' type='checkbox' id='garonsite_24' value='OK' $ckos2></td>
    <td>24 MESES</td>
    <td>R$ <input name='v_garonsite_24' type='text' id='v_garonsite_24' size='5' maxlength='5' value='$xv_garonsite_24'>
    </td>
  </tr>
</table>
...
");
if ($Action == "update" or $Action == "plano"){
echo("
<table width='550' border='1' align='center' cellpadding='0' cellspacing='0'>
  <tr>
    <td><form name='form1' method='post' action='$PHP_SELF?Action=plano'>
      <table width='100%' border='0' align='center'>
        <tr bgcolor='#FF9933'>
          <td width='20'>&nbsp;</td>
          <td width='293'><strong>PLANO TELEFONIA </strong></td>
          <td width='200' align='left'><strong>DESCONTO (R$) </strong></td>
          <td width='200' align='left'><strong>PAGAR COMISS&Atilde;O?</strong></td>
        </tr>
  ");

  	$prod->listProdSum("codplano, plano" , "produtos_planos", "hist <> 'S'", $array45, $array_campo45, "order by plano" );
	 ...
	 for($i = 0; $i < count($array45); $i++ ){
			...
			$codplano = $prod->showcampo0();
			$plano = $prod->showcampo1();
			$prod->clear();
			$prod->listProdU("desconto, hist, scomissao", "produtos_planos_relacao", "codplano='$codplano' and codprod = '$codprodold'");
			$desconto = $prod->showcampo0();
			$descontof = number_format($desconto,2,',','.');
			$hist = $prod->showcampo1();
			$comissao = $prod->showcampo2();
			if ($hist == 'S'){$ck = "checked"; }else{$ck = "";}

  echo("
    <tr>
      <td><input type='checkbox' name='codplano_sel[$i]' value='$codplano' $ck></td>
      <td>$plano</td>
      <td align='left'>R$
      <input name='desconto_sel[$i]' value = '$descontof' type='text' size='10' onKeyUp='this.value = adjust(this);'></td>
         <td align='left'><select name='comissao_sel[$i]'>
           <option value='S' selected>S</option>
           <option value='N'>N</option>
           <option value='$comissao' selected>$comissao</option>
                  </select></td>
    </tr>
    ");
    }
   ...
   <tr>
          ...
          <td>&nbsp;</td>
          <td align='left'>&nbsp;</td>
          <td align='left'><input type='submit' name='Submit' value='ADICIONAR'></td>
   </tr>
      </table>
 ...
 echo("
    <BR>
<font  face='Verdana, Arial, Helvetica, sans-serif' size='2' color='#FF6600'><b>:: SELECIONAR PRODUTOS COM RESTRI&Ccedil;&Atilde;O</b></font><br>
<table width='100%'  border='0' align='center' cellpadding='0' cellspacing='0' style='border-style: solid; border-color: black; border-width: 1px 1px 1px 1px;'>
  <tr>
    <td><iframe src='inicioprod_select.php?codcatselect=$codcatold&codprodselect=$codprodold' name='iprodutoadcmicro' width='100%' marginwidth='0' height='400' marginheight='0' scrolling='yes' frameborder='0'></iframe></td>
  </tr>
</table>
	");
...
echo("
    ...
    <td align='center' width='15%' height='0'> 
	 ...
	 <td align='center' width='15%' height='0'> 
      <div align='center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>PVD</font></b></div>
    </td>
	...
	");
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			$kit = $prod->showcampo6();
			$pvd = $prod->showcampo7();
			$pvd = number_format($pvd, 2,',', '.');
			$descres = $prod->showcampo8();
		...
		echo("
		<tr bgcolor='$cor'> 
			<td width='10%' height='0' align='center'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'><b>$codprod</b></font></td>
			<td width='35%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>$nomeprod</font><br><font face='Verdana, Arial, Helvetica, sans-serif' size='1' color='#808080'>$descres</font></td>
			...
			<td align='center' width='15%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>$pvd</font></td>
