...
if ($Actionsec == "list"){
		...
		$prod->listProdSum("codprod, nomeprod, pvv, pva, datauc, unidade, hist, kit, lib_linha, produtos.codcat, produtos.codsubcat , descres, garext_12, garext_24, porc_garext_12, porc_garext_24, ponto, tipo_gar, gar_um, chkcb", "produtos, categoria, subcategoria", $condicao2, $array, $array_campo, $parm );
...
echo("
    ...
    <tr>
      <td width='100%'>
        ...
        TABELA</font></a></b></td>
			...
			<td width='5%'><font size='1' face='Verdana'><img border='0' src='images/print.gif' width='36' height='21'></font></td>
			<td width='20%'><b><a href='iniciotabelapreco_imp.php?Action=list&codprod=$codprod&codempselect=$codempselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&hist=0' target = '_black'><font size='1' face='Verdana'>IMPRIMIR TABELA COMPLETA</font></a></b></td>
  ...
  <tr>
    ...
    <td  align='center' width='5%' height='0'> 
      <div align='center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>GAR<BR>(mês)</font></b></div>
    </td>
    ...
    <td align='center' width='10%' height='0'> 
	 ...
	 <td align='center' width='5%' height='0'>
      <div align='center'><b><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>VERIF<br>CB</font></b></div>
    </td>
	...
	");
	 ...
	 for($i = 0; $i < count($array); $i++ ){
			...
			$tipo_gar = $prod->showcampo17();
			$gar_um = $prod->showcampo18();
			$chkcb = $prod->showcampo19();
			
			$prod->clear();
			$prod->listProdU("fatorata, fatorvar", "estoque", "codprod=$codprod and codemp=$codempselect");
			$fatormult_ata = $prod->showcampo0();
			$fatormult_var = $prod->showcampo1();
			...
			if ($garext_12=="OK" or $garext_24=="OK")
            {
                $estrela = 1;
                $preco12 = ($pvv * ($porc_garext_12 / 100));
                $fpreco12 = number_format($preco12, 2,',', '.');
                $preco24 = ($pvv * ($porc_garext_24 / 100));
                $fpreco24 = number_format($preco24, 2,',', '.');
                $layinfo = "<a class='layinfo' href='#'><img src='".DIR_IMAGES."/estrela.gif' border='0'><span><b>Garantia Estendida</b><br>+12 meses R$ $fpreco12<br>+24 meses R$ $fpreco24</span></a>";
            }
            else
            {
                $estrela = 0;
			...
			$pvvf = number_format($pvvcjtotal,2,',','.');
			$pvacjtotal = $pvacjtotal * $fatorusertabela;
			$pvaf = number_format($pvacjtotal,2,',','.');
			}
...
$xcodsubcat = $codsubcat;
	...
	if ($kit_no <> 1 and $linha_no <> 1){
		...
		<tr bgcolor='$cor'> 
			...
			<td width='5%' height='0' align='center'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'><b>$tipo_gar</b><br>$gar_um</font></td>
			<td align='center' width='15%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>$pvvf</font>"); if($estrela == "1"){echo $layinfo;} echo ("</td>
			<td align='center' width='10%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>$alarm</font></td>
			<td align='center' width='5%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>$ponto_prod</font></td>
			<td align='center' width='5%' height='0'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>"); if($chkcb == "Y"){echo "S";}else{echo "N";} echo ("</font></td>
			<td align='center' width='10%' height='0'><font size='1'><b><a href='$PHP_SELF?Action=update&codprod=$codprod&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&retlogin=$retlogin&connectok=$connectok&pg=$pg&codempselect=$codempselect'><font face='Verdana, Arial, Helvetica, sans-serif'>Detalhes
			  </font></b></a></font></td>
