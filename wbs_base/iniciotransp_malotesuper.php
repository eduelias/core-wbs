...
switch ($Action) {
    ...
    case "insert":
			...
			$dataatual = $prod->gdata();
				...
				$prod->clear();
				$prod->setcampo0("");
				$prod->setcampo1($codvendselect);
				$prod->setcampo2("NO");  //SOL
				$prod->setcampo3("NO");  //COL
				$prod->setcampo4("NO");  //ENT
				$prod->setcampo5($dataatual);  //DATAMAL
				$prod->setcampo9('NOVO MALOTE');
				$prod->setcampo10($dataatual);  //DATASTATUS
				$prod->setcampo7("N");  //HIST
				$prod->addProd("transp_malote", $ureg);

				// ATUALIZA STATUS DA TABELA
				$prod->setcampo0("");
				$prod->setcampo1($ureg);
				$prod->setcampo2($dataatual);
				$prod->setcampo3('NOVO MALOTE');
				$prod->setcampo4($login);
				$prod->addProd("transp_malote_status", $ureg1);

				$prod->listProdU("email_cel, email_msg", "vendedor", "codvend='$codvendselect'");
				...
				if ($email_msg == "Y"){
					...
					$mail->setFrom('ipasoft');
					$mail->setSubject('NOVA SOLICITAÇÃO DE MALOTE');
					$mail->setText($text);
...
if ($Actionsec == "list"){
		...
		if ($codpesq){
			...
			$condicao3 .= " transp_malote.codmal='$codpesq' and";
				...
				#$condicao3 .= " transp_malote.codvend='$codvendselect'";
...
<div align='center'>
      ...
      <td width='100%'>
        <table border='0' width='100%' cellspacing='0' cellpadding='0'>
          <tr>
            ...
            <td width='50%'><b><a href='$PHP_SELF?Action=list&codprod=$codprod&codempselect=$codempselect&desloc=$desloc&palavrachave=$palavrachave&operador=$operador&pagina=$pagina&codprodpesq=$codprodpesq&retlogin=$retlogin&connectok=$connectok&pg=$pg&hist=$hist&nomevendpesq=$nomevendpesq'><font face='Verdana, Arial, Helvetica, sans-serif' size='1'>ATUALIZAR
              TABELA</font></a></b></td>
          <td width='40%'>
 ...
 <div align='center'>
    ...
    <table border='0' width='550' cellspacing='1' cellpadding='2'>
      <tr>
        <td width='100%'>
          <p align='left'><font face='Verdana' size='1' color='#FF0000'><b>OBS.1:</B> No caso de <b>CANCELAMENTO</b> do Malote entrar em contato diretamente com a transportadora. Falar com Andressa (32) 3217-3500. </font></p>
		</td>
      </tr>
    </table>
    </center>
