...
function barcode_encode_genbarcode($code,$encoding){
    ...
    $fp=popen($genbarcode_loc." ".escapeshellcmd($code)." ".escapeshellcmd(stripslashes($encoding)), "r");
    if ($fp){
	$bars=fgets($fp, 1024);
	$text=fgets($fp, 1024);
...
function barcode_encode($code,$encoding){
	...
	print "Can't find <U>$genbarcode_loc</U><BR>\n";
    }
    return $bars;
}
...
function barcode_print($code, $encoding="ANY", $scale = 2 ,$mode = "png" ){
    ...
    elseif (eregi($mode,"^(html|htm)$")) print barcode_outhtml($bars[text],$bars[bars],$scale,$y, $space);
