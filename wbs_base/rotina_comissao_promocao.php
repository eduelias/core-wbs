...
// ACOES PRINCIPAIS DA PAGINA
		...
		$prod->listProdSum("pedido.codped", "pedido, pedidostatus", "dataped > '2005-08-01' AND dataped <  '2005-08-31' AND ( pedido.contrato =  'OK' OR pedido.contrato =  'DC' ) AND pedido.caixa =  'OK' AND pedido.codped = pedidostatus.codped AND statusped =  'CAIXA' AND datastatus <=  '2005-09-04'", $array, $array_campo, "order by datastatus");
		...
		for($i = 0; $i < count($array); $i++ ){
			...
			$prod->listProdU("mlb, vc, fatorcomissao, tipo, pedido.codvend, codbarra, vpv, nome, mcv, codgrp", "pedido, vendedor", "codped=$codped and pedido.codvend=vendedor.codvend");
			...
			$ped_codgrp = $prod->showcampo9();
			$fator_corr = 0.033;
			if ($ped_codgrp == 46){$ped_fatorcomissao = 0;}
		...
		if ($ped_mcv <= 50){
			...
			*/
				...
				if ($ped_tipo == 'R'){
					$prod->setcampo6("2005-09-05 00:00:00");
					$prod->setcampo7("2005-09-05 00:00:00");
				}else{// padrao dia 30 do mes da pesquisa
					$prod->setcampo6("2005-08-25 00:00:00");
					$prod->setcampo7("2005-08-25 00:00:00");
				}	
					$prod->setcampo8("N");
