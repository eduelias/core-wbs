...
$rs_lista = $db->query_db("SELECT pedido.codped, SUM( if( DATE_FORMAT( dataped, '%Y-%m' ) = DATE_FORMAT( NOW( ) , '%Y-%m' ) , vt, 0 ) ) , pedido.codvend, vendedor.nome, SUM( if( DATE_FORMAT( dataped, '%Y-%m' ) = DATE_FORMAT( NOW( ) , '%Y-%m' ) , if( modelo_ped = 'RDD', vt, 0 ) , 0 ) ) , SUM( if( DATE_FORMAT( dataped, '%Y-%m' ) = DATE_FORMAT( NOW( ) , '%Y-%m' ) , if( modelo_ped = 'RDST', vt, 0 ) , 0 ) ) , SUM( if( DATE_FORMAT( dataped, '%Y-%m' ) = DATE_FORMAT( NOW( ) , '%Y-%m' ) , if( modelo_ped = '', vt, 0 ) , 0 ) ) FROM pedido , vendedor  WHERE pedido.codvend = vendedor.codvend  AND (pedido.codemp =14 or pedido.codemp = 15) AND cancel <> 'OK' AND dataped >= DATE_SUB( NOW( ) , INTERVAL 120 DAY )  and (modelo_ped = 'RDD' or modelo_ped = 'RDST' or (modelo_ped = '' and vendedor.tipo = 'R')) and ped_transf <> 'OK'  ".$condpesq." GROUP by pedido.codvend","SQL_NONE","N");
...
$rs_lista1 = $db->query_db("SELECT pedido.codped, SUM( if( DATE_FORMAT( dataped, '%Y-%m' ) = DATE_FORMAT( NOW( ) , '%Y-%m' ) , vt, 0 ) ) , SUM( if( DATE_FORMAT( dataped, '%Y-%m' ) = DATE_FORMAT( NOW( ) , '%Y-%m' ) , if( modelo_ped = 'DST', vt, 0 ) , 0 ) ) , codcliente FROM pedido WHERE  (pedido.codemp =14 or pedido.codemp = 15) AND cancel <> 'OK' AND dataped >= DATE_SUB( NOW( ) , INTERVAL 120 DAY )  and (modelo_ped = 'DST' ) and ped_transf <> 'OK' ".$condpesq1." GROUP by pedido.codcliente","SQL_NONE","N");
...
if ($rs_lista1) {
		...
		$cont_reg1= $rs_lista_verif1->fields[0];
		...
		while (!$rs_lista_prod1->EOF) { 
			...
			$cont_reg_prod1= $rs_lista_verif2->fields[0];
