...
if ($rs_lista) {
		while (!$rs_lista->EOF) { 
			...
			$rs_custo = $db->query_db("SELECT   (SUM( ifnull( qtde * ( SELECT (custo_ger) FROM ocprod, oc WHERE ocprod.codoc = oc.codoc AND codped_transf =0 AND datacompra <= '".$rs_lista->fields[2]."' AND codprod = pedidoprod.codprod ORDER BY datacompra DESC  LIMIT 0 , 1 ) , if (qtde * ( SELECT puc FROM produtos WHERE codprod = pedidoprod.codprod ) , qtde * ( SELECT puc FROM produtos WHERE codprod = pedidoprod.codprod ),  qtde * ( SELECT (custo_ger) FROM ocprod, oc WHERE ocprod.codoc = oc.codoc AND codped_transf =0 AND datacompra >= '".$rs_lista->fields[2]."' AND codprod = pedidoprod.codprod ORDER BY datacompra ASC  LIMIT 0 , 1 )) )) )  FROM pedidoprod WHERE codped = '".$rs_lista->fields[0]."' GROUP by pedidoprod.codped","SQL_NONE","N");
...
$rs_st = $db->query_db("SELECT SUM(valorpago) FROM fin_cap WHERE (opcaixa = '700.17' or opcaixa = '700.19' or opcaixa = '700.20') and  datavenc like '$ano-$mes%' ","SQL_NONE","N");
...
if ($rs_lista) {
		while (!$rs_lista->EOF) { 
			...
			$custo_total = $custo_total + $rs_st->fields[0];
			$vpp_geral = $vpp_geral + $vpp_total;
			$cartao_geral = $cartao_geral + $cartao_total;
			$custo_geral = $custo_geral + ($custo_total);
			$boleto_geral = $boleto_geral + $boleto_total;
...
<tr bgcolor="#FFFF66">
  ...
  <tr>
    <td><strong>ST DISTRIBUIÇÃO</strong></td>
    <td><div align="left"><? echo $db->fvalor($rs_st->fields[0]); ?></div></td>
    <td><div align="left"></div></td>
    <td><div align="left"></div></td>
    <td><div align="center"></div></td>
    <td><div align="left"></div></td>
