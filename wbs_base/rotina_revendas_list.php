...
$prod->clear();
$prod->listProdSum("codvend, nome, doc, tipocliente, codsuper, dataini", "vendedor", "tipo = 'R' AND block = 'N' AND (codsuper <> '22' AND codsuper <> '24' AND codsuper <> '25' AND codsuper <> '33')", $arrayx, $array_campox, "ORDER BY nome ASC");
...
*/
    ...
    $prod->clear();
    $prod->listProdU("nome, cidade, dddtel1, tel1, email, dddtel2, tel2, cep, endereco, numero, complemento, bairro","clientenovo", "$condicaopesq1");
	...
	$cep_list[$i] = $prod->showcampo7();
	$endereco_list[$i] = $prod->showcampo8();
	$numero_list[$i] = $prod->showcampo9();
	$compl_list[$i] = $prod->showcampo10();
	$bairro_list[$i] = $prod->showcampo11();
...
<table  border="1" cellpadding="1" cellspacing="2">
  <tr>
    ...
    <td class="titulo_tabela">Cidade/CEP</td>
  </tr>
<?	for($i = 0; $i < count($arrayx); $i++ ){ ?>
  <tr>
...
<? echo $dataini_list[$i]; ?></td>
    ...
    <td valign="top" class="resul_tabela"><? echo $cidade_list[$i]; ?><br><? echo $cep_list[$i]; ?></td>
  </tr>
  <tr>
    <td colspan="6" class="resul_tabela"><? echo $endereco_list[$i]; ?>, <? echo $numero_list[$i]; ?> - <? echo $compl_list[$i]; ?> - <? echo $bairro_list[$i]; ?></td>
  </tr>
<?php } ?>
</table>
</body>
