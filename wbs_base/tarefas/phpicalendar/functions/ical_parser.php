...
/* patch to allow 401 authentication for Tasks iCalendar
...
/____________________________________________________*/
		...
		if ($ifile == false) {
			function prompt_401() {
				require('tasks_ical_login.php');
				die();
			}
			function open_w_401($username, $password) {
				global $filename;
				if (strstr($filename, "http://")) {
					$filename = 'http://'.urlencode($username).':'
					                     .urlencode($password).'@'
					                     .substr($filename, 7);
				}
				else if (strstr($filename, "https://")) {
					$filename = 'http://'.urlencode($username).':'
					                     .urlencode($password).'@'
					                     .substr($filename, 8);
				}
				else if (strstr($filename, "webcal://")) {
					$filename = 'webcal://'.urlencode($username).':'
					                       .urlencode($password).'@'
					                       .substr($filename, 9);
				}
				$ifile = @fopen($filename, "r");
			...
			if (!empty($_POST['username_401']) && !empty($_POST['password_401'])) {
				$ifile = open_w_401(stripslashes($_POST['username_401']), stripslashes($_POST['password_401']));
				if (!$ifile) {
					prompt_401();
				}
			}
			if (!$ifile && !empty($_SERVER['PHP_AUTH_USER']) && !empty($_SERVER['PHP_AUTH_PW'])) {
				$ifile = open_w_401(stripslashes($_SERVER['PHP_AUTH_USER']), stripslashes($_SERVER['PHP_AUTH_PW']));
				if (!$ifile) {
					prompt_401();
				}
			}
...
// replacing the password so it isn't shown on the error page.
// Alex King, http://www.alexking.org/

		if ($ifile == FALSE) exit(error($error_invalidcal_lang, @str_replace($_SERVER['PHP_AUTH_PW'], 'xxxxx', $filename))); 
		$nextline = fgets($ifile, 1024);
		if (trim($nextline) != 'BEGIN:VCALENDAR') exit(error($error_invalidcal_lang, @str_replace($_SERVER['PHP_AUTH_PW'], 'xxxxx', $filename)));
