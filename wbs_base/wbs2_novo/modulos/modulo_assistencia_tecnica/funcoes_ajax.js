...
{
	var theURL = base + "";
	alert ('laudo'+ codlaudo_select+ '');
	alert("codlaudo_select: "+codlaudo_select+" dataArray.length: "+dataArray.length+"");
	var codlaudo = codlaudo_select;
	doit(theURL, 'opcoes1', param+'&Action=BuscaSrvExec&menu_not=1&codlaudo='+codlaudo+'', 'GET', 'opcoes1');
	ajax.onCompletion = function() {
		...
		if(dataArray.length > 0)
		{
			...
			for(var i = 0 ; i < dataArray.length ; i++)
			{
				...
				var descricao = item.getElementsByTagName("codsrv_exec_select")[0].firstChild.nodeValue + " - " + item.getElementsByTagName("descricao")[0].firstChild.nodeValue;
