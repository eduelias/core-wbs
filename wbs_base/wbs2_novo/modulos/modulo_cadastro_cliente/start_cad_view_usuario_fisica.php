...
+--------------------------------------------------------------
|  arquivo:     start_cad_view_usuario_fisica.php
|  template:    cad_view_usuario_fisica.htm
+--------------------------------------------------------------
*/
...
switch ($Action) {
    case "Procurar" :
        ...
        if ($codcliente != "") {
			...
			$difdias_dono = $prod->numdias($xdatadono,$datahoje);// ALTERADO

			if ($difdias_dono > 90){$block_dono = 0;}// ALTERADO
    		else{
              if ($xlogincad <> $login_vend and $list_cli_padrao <> 'Y'){$block_dono = 1;}
...
/*			$datanew = str_replace('/','',$frmNasc);
			...
			*/
            ...
            #header("Location: http://" . $_SERVER['HTTP_HOST']. dirname($_SERVER['PHP_SELF']). $bar. "wbs2/redirect.php?pg=$pg&pg_ped=$pg_ped&txtreload=cadalt");
...
include ("templates/cad_view_usuario_fisica.htm");
