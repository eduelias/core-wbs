...
$rs_count = $db->query_db("SELECT count(*) AS total_reg FROM vendedor, clientenovo WHERE $condicaopesq tipo = 'R' AND ((doc = cnpj) OR (doc = cpf))","SQL_NONE","N");
...
$rs_lista = $db->query_db("SELECT vendedor.codvend AS codlogin, vendedor.nome AS login, doc, clientenovo.nome AS nome, cidade, estado, email, cep, dddtel1, tel1, dddtel2, tel2, endereco, numero, complemento, bairro FROM vendedor, clientenovo WHERE $condicaopesq tipo = 'R' AND ((doc = cnpj) OR (doc = cpf)) AND block = 'N' ORDER BY vendedor.nome LIMIT $inicio_pesq, $acrescimo","SQL_NONE","N");

$numRows = $rs_lista->_numOfRows;

$db->disconnect();

include ("templates/controle_revenda_rma_index.htm");
