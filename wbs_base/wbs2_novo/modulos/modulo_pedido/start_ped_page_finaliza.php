...
switch ($Action)
{
	...
	// CRIA PRODUTOS DO PEDIDO
        $pontot_prod = 0;
        $prod->clear();
        $prod->listProdgeral("pedidoprodtemp", "codped=$codpedselect and codprod <> 0", $array, $array_campo, "" );
        for($i = 0; $i < count($array); $i++ )
        {
        $garumprod_ped = 0;
        $garcjprod_ped = 0;
		...
		// CRIA RESERVA NA TABELA ESTOQUE
	        $prod->clear();
			$prod->upProdU("estoque", "reserva = reserva + $qtdeprod_ped", "codprod = '$codprod_ped' AND codemp = '$codemp_ped'");
        ...
        break;
       	...
       	case "finaliza":
			...
			$titulo = "Pedido";
	               	$Action = "ped_grava";
	               	$prod->clear();
			$prod->listProdU("pendfpg, contrato", "pedido", "codped = '$codpedselect'");
			$pendfpg = $prod->showcampo0();
			$contrato = $prod->showcampo1();
