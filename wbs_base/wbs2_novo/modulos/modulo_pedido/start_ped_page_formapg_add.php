...
#$valortabela_real = "2.312,00";
	...
	$prod->clear();
	$prod->listProdU("pedidotemp.codemp, pedidotemp.codvend, tipo, fatorusertabela, codcliente, vpp,vpv, mlb, mcv, tipo_vge, sj10x","pedidotemp, vendedor", "codped='$codpedselect' AND pedidotemp.codvend=vendedor.codvend");
...
#echo("codcli = $codcliente");
    ...
    $prod->clear();
    $prod->listProdSum("opcaixa10x","pedidoprodtemp, produtos", "codped='$codpedselect' and opcaixa10x <> '' AND produtos.codprod = pedidoprodtemp.codprod", $array64, $array_campo64 , "group by produtos.codprod");
	   for($j = 0; $j < count($array64); $j++ ){
		...
		#echo"op<pre>";print_r($xopcaixashow);
    }else{
	...
	$xopcaixashow = explode(":", $opcaixa);
	$xopcaixashow = array_intersect($xopcaixashow, $formapg_padrao);
	$xopcaixashow = array_values($xopcaixashow);
	#echo"op <PRE>";print_r($xopcaixashow);
	#echo"op <PRE>";print_r($formapg_padrao);
	...
	// LISTA OS REGISTROS
	 for($l = 0; $l < count($xopcaixashow); $l++ ){
		...
		if ($xopcaixashow[$l] <> ""){
		$prod->listProdU("descricao, tipo, fabrica, hist","formapg", "opcaixa='$xopcaixashow[$l]'");
		$descricao[$l] = $prod->showcampo0();
		$tipo_cel[$l] = $prod->showcampo1();
		$fabrica[$l] = $prod->showcampo2();
		$hist_op[$l] = $prod->showcampo3();		
		}
	}// FOR
   ...
   switch ($Action)
{
	case "grava":
		...
		for($i = 1; $i <= $nump; $i++ ){
			...
			// VERIFICA PENDENCIA DE DADOS
			/*
			if($formapg[$i] == '02.55' or $formapg[$i] == '02.35' or $formapg[$i] == '02.36' or $formapg[$i] == '02.59'){
...
// VERIFICA SE DENTRE AS PARCELAS EXISTE ALGUMA QUE POSSUI DINHEIRO OU CARTAO OU VISAELECTRON OU FINANCIAMENTO
#if (($formapg[$i] =='02.00' or $formapg[$i] =='02.30' or $formapg[$i] =='02.31' or $formapg[$i] =='02.32' or $formapg[$i] =='02.33' or $formapg[$i] =='02.34' or $formapg[$i] =='02.40' or $formapg[$i] =='02.41' or $formapg[$i] =='02.42' or $formapg[$i] =='02.43' or $formapg[$i] =='02.44' or $formapg[$i] =='02.45' or $formapg[$i] =='02.46' or $formapg[$i] =='02.47' or $formapg[$i] =='02.48' or $formapg[$i] =='02.49' or $formapg[$i] =='02.50' or $formapg[$i] =='02.51' or $formapg[$i] =='02.52'  or $formapg[$i] =='02.53'  or $formapg[$i] =='02.54' or $formapg[$i] =='02.55' or $formapg[$i] =='02.56') and $mcv >= 0){$dindim = $dindim*1;}else{$dindim = $dindim*0;}
		
		//VERIFICA SE A FORMA DE PAGAMENTO NECESSITA DE APROVACAO
		$prod->clear();
        $prod->listProdU("lib_aprov","formapg", "opcaixa = '".$formapg[$i]."'");
		...
		$prod->clear();
        $prod->listProdU("lib_contrato","formapg", "opcaixa = '".$formapg[$i]."'");
		...
		$prod->clear();
        $prod->listProdU("aguarda_comp","formapg", "opcaixa = '".$formapg[$i]."'");
       ...
       // APLICA RESTRICAO DE CONTRATO
		$prod->clear();
        $prod->listProdU("COUNT(*)","pedidoprodtemp, produtos", "codped='$codpedselect' and produtos.lib_contrato !='S' and pedidoprodtemp.codprod=produtos.codprod ");
		...
		$condicao_ped .= "prontaentr = 'NO', "; // PRONTA ENTREGA
   		...
   		$condicao_ped .= "inicio_sep = 'NO' "; // INICIO DA SEPARACAO
