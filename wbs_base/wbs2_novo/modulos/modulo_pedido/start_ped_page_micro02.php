...
$codplacaselect =  $var_select1[0]; // CODIGO DA PLACA
$codcjselect = $var_select1[1]; // CODIGO DA PLATAFORMA
$codtipocjselect = $var_select1[2]; // CODIGO DO TIPOCJ
if (!$codtipocjselect){$codtipocjselect=1;}
...
$prod->listProdSum("codcat, nomecat", "categoria", "codcat <> 4 and codcat <> 46 and codcat <> 71 and codcat <> 72", $array_pesq, $array_campo_pesq, "ORDER BY nomecat");
...
switch ($Action)
{
	...
	if ($palavrachave <> ""){
		...
		$condicao2 .= " AND (produtos.codcat <> 4 AND produtos.codcat <> 71 AND produtos.codcat <> 46 AND produtos.codcat <> 72)";
	}
...
if ($condicao2 <> ""){
	...
	if ($Action_alter == "altera"){
	
	$tipocj_novo = $codtipocjselect;
        $prod->delProd("pedidoprodtemp", "codped = '$codpedselect' and tipocj ='$codtipocjselect'");

	}else{

	$prod->clear();
	$prod->listProdU("MAX(tipocj)+1","pedidoprodtemp", "codped=$codpedselect");
	$tipocj_novo = $prod->showcampo0();
	if ($tipocj_novo == ""){$tipocj_novo = 1;}
	
	}// ALTERACAO DA CONFIGURACAO
	...
	// TEMPLATE
	include ("templates/ped_page_micro02.htm");
