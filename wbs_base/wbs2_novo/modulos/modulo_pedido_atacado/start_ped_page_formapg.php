   ...
   switch ($Action)
{
	case "grava":
		...
		$condicao_ped .= "vt = $vt, ";
		$condicao_ped .= "vpv = $vpv, ";
		$condicao_ped .= "vpp = $vpp, ";
		$condicao_ped .= "mlb = $mlb, ";
		$condicao_ped .= "mcv = $mcv, ";
		$condicao_ped .= "vc = $vc, ";
		...
		$condicao_ped .= "mlb_real = $mlb_real, ";
		$condicao_ped .= "mcv_real = $mcv_real, ";
...
if ($orcamento == 1){
		...
		if ($tipo_pessoa == "F"){
		        if (dirname($_SERVER['PHP_SELF']) == "/"){$bar = "";}else{$bar = "/";}
			header("Location: http://" . $_SERVER['HTTP_HOST']. dirname($_SERVER['PHP_SELF']). $bar . "restrito.php?pg=$pg&pg_ped=16&codpedselect=$codpedselect&tipo_p=$tipo_p");
		}else{
			if (dirname($_SERVER['PHP_SELF']) == "/"){$bar = "";}else{$bar = "/";}
			header("Location: http://" . $_SERVER['HTTP_HOST']. dirname($_SERVER['PHP_SELF']). $bar. "restrito.php?pg=$pg&pg_ped=16&codpedselect=$codpedselect&tipo_p=$tipo_p");
		}
