...
// PESQUISA NUMERO DE OC DE COMPRA
	...
	$prod->clear();
	$prod->listProdU("SUM(qtdecomp) - $reserva[$k] as est_oc,  dataprevcheg ","ocprod, oc", "codprod=$codprod[$k] and ocprod.codemp=$codempselect and oc.hist <> 1 and oc.codoc=ocprod.codoc GROUP by codprod ORDER by dataprevcheg DESC");
...
if ($prod_selected[$k] <> 0 and $restricao_view[$k] <> 1){
	...
	$prod->clear();
	$prod->listProdU("nomeprod, (pvacj*fatorata*$fatorusertabela),  (pvvcj*fatorvar*$fatorusertabela), ordem","produtos, estoque, categoria", "produtos.codprod=$codplacaselect AND produtos.codprod=estoque.codprod AND produtos.codcat=categoria.codcat");
