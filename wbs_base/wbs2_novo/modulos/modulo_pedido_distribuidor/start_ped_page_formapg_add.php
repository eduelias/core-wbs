...
#echo("codcli = $codcliente");
   ...
   #echo "cliente = $codcliente";
	...
	if($tipo_preal == 1) {$array_ata = "02.00:02.01:02.17:02.18";}
	#if($tipo_preal != 1) {$array_ata = "02.01:02.04";}
	if($tipo_preal != 1) {if ($vpp > 300) {$array_ata = "02.01:02.04";}else{$array_ata = "02.01";}}
	...
	$prod->listProdSum("datavenc, vp, codparcped ", "pedidoparcelastemp", "codped = '$codpedselect'", $arrayx, $array_campox, "ORDER BY datavenc");
        ...
        $count_soft = $prod->showcampo0();
		 ...
		 // PESQUISA POR PPB NO PEDIDO
  		$prod->clear();
        $prod->listProdU("COUNT(*)","pedidoprodtemp, produtos", "codped='$codpedselect' and produtos.chk_ppb = 'S' and pedidoprodtemp.codprod=produtos.codprod ");
        $count_ppb = $prod->showcampo0();
		
		if ($count_ppb > 0 and $abcestado <> 'MG' and $tipo_pis == 'R' and $codempselect <> 14){$bloqueio_ppb = 1;}else{$bloqueio_ppb = 0;}
		#echo "($count_ppb > 0 and $abcestado <> 'MG' and $tipo_pis == 'R' and $codempselect <> 14";      
...
if ($formapg[$i] =='02.00'){$dindim = $dindim*1;}else{$dindim = $dindim*0;}
		...
		$condicao_ped .= "modelo_ped = 'DST', "; // TIPO DE PEDIDO DST = DISTRIBUICAO
