...
#$codcliente = 140;
	...
	$prod->clear();
	$prod->listProdU("pedidotemp.codemp, pedidotemp.codvend, tipo, fatorusertabela, ped_bol","pedidotemp, vendedor", "codped='$codpedselect' AND pedidotemp.codvend=vendedor.codvend");
	$codempselect = $prod->showcampo0();
	$codvendselect = $prod->showcampo1();;
	$tipovend = $prod->showcampo2();
	$fatorusertabela = $prod->showcampo3();
	$ped_bol = $prod->showcampo4();
   ...
   switch ($Action)
{
	case "grava":
	 	...
	 	$prod->clear();
		$prod->listProdU("codcliente, clientenovo.nome","vendedor LEFT JOIN clientenovo ON vendedor.doc = if( vendedor.tipocliente = 'F', clientenovo.cpf, clientenovo.cnpj )", "codvend = ( SELECT codvend FROM pedidotemp WHERE codped ='$codpedselect' )");
		...
		$condicao_ped = "codcliente = $codcliente, ";
		$condicao_ped .= "vt = $vt_mdped, ";
		$condicao_ped .= "vt_real = $valortabela_real, ";
		...
		$condicao_ped .= "mlb_dist = '$mlb_dist'";
...
if ($orcamento == 1){
		...
		if ($tipo_pessoa == "F"){
		        if (dirname($_SERVER['PHP_SELF']) == "/"){$bar = "";}else{$bar = "/";}
			header("Location: http://" . $_SERVER['HTTP_HOST']. dirname($_SERVER['PHP_SELF']). $bar . "restrito.php?pg=$pg&pg_ped=12&codpedselect=$codpedselect&tipo_p=$tipo_p");
		}else{
			if (dirname($_SERVER['PHP_SELF']) == "/"){$bar = "";}else{$bar = "/";}
			header("Location: http://" . $_SERVER['HTTP_HOST']. dirname($_SERVER['PHP_SELF']). $bar. "restrito.php?pg=$pg&pg_ped=13&codpedselect=$codpedselect&tipo_p=$tipo_p");
		}
}
