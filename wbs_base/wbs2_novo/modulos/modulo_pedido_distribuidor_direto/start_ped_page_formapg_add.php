...
#$valortabela_real = "2.312,00";
	...
	$prod->clear();
	$prod->listProdU("pedidotemp.codemp, pedidotemp.codvend, tipo, fatorusertabela, codcliente, vpp,vpv, mlb, mcv, tipo_vge, sj10x, mlb_dist, codvend_dist, codvend_user, mlb_dist","pedidotemp, vendedor", "codped='$codpedselect' AND pedidotemp.codvend=vendedor.codvend");
	...
	$mlb_dist = $prod->showcampo11();
	$codvend_dist = $prod->showcampo12();
	$codvend_user = $prod->showcampo13();
	$mlb_dist = $prod->showcampo14();
	
	$mlb_rev=$mlb-$mlb_dist;
	
		$prod->listProdU("tipo_supervisor","vendedor_usuario", "codvend_user = '$codvend_user'");
		$tipo_supervisor = $prod->showcampo0();
	
	if ($vpp < 1200){
		$porc_dist_retira = ($mlb_dist/$mlb)*100;
		$porc_dist_entrega = ($mlb_dist/($mlb-10))*100;
		$vfrete = 18;
	}else{
		$porc_dist_retira = ($mlb_dist/$mlb)*100;
		$porc_dist_entrega = ($mlb_dist/($mlb))*100;
		$vfrete = 0;
	}
...
#echo("codcli = $codcliente");
   ...
   #echo "cliente = $codcliente";
	...
	if($tipo_p != 1) {$array_ata = "02.01:02.04";}
	$show_array_ata = explode(":", $array_ata);
	#print_r($xopcaixashow1);
	#print_r($show_array_ata);
	...
	$prod->clear();
	$prod->listProdSum("datavenc, vp, codparcped, tipo ", "pedidoparcelastemp", "codped = '$codpedselect'", $arrayx, $array_campox, "ORDER BY datavenc");
		...
		for ($j = 0; $j < count($arrayx); $j++)
		{
			...
			$tipoparc[$j+1] = $prod->showcampo3();
			
			#echo "AQUI = ".in_array("02.04", $xopcaixashow1);
			
			if ($tipoparc[$j+1] == '02.04' and in_array("02.04", $xopcaixashow1) == 1 ){$tipoparc[$j+1] = '02.04';}else{if($tipoparc[$j+1] == '02.04'){$tipoparc[$j+1]= "";}else{$tipoparc[$j+1] = $tipoparc[$j+1];}}		
			#$tipoparc[$j+1] = array_intersect($xopcaixashow1, $tipoparc_bol);
			
			$vpf[$j+1] = $prod->fpreco($vp[$j+1]);
			$datavencf[$j+1] = $prod->fdata($datavenc[$j+1]);
			
			$prod->clear();
			$prod->listProdU("descricao, tipo, fabrica","formapg", "opcaixa='".$tipoparc[$j+1]."'");
			$descricao_parc[$j+1] = $prod->showcampo0();
			

		}//FOR
		#$tipoparc = array_intersect($xopcaixashow1, $tipoparc);
		#print_r ($tipoparc);
...
if ($formapg[$i] =='02.00'){$dindim = $dindim*1;}else{$dindim = $dindim*0;}
		...
		$prod->listProdU("codcliente","clientenovo", "if( tipocliente = 'F', cpf = (SELECT doc FROM vendedor WHERE codvend = ( SELECT codvend FROM pedidotemp WHERE codped ='$codpedselect' ) ) , cnpj = ( SELECT doc FROM vendedor WHERE codvend = ( SELECT codvend FROM pedidotemp WHERE codped ='$codpedselect' ) ) ) ");
        $codcliente_aval = $prod->showcampo0();
    	...
    	$condicao_ped .= "codvend_cm = '$codvend_cm_select', ";
			if ($modoentr == 'ENTREGA' ){$vfrete_final = $vfrete;}else{$vfrete_final = 0;}
		$condicao_ped .= "mlb = mlb-$vfrete_final, ";
		$condicao_ped .= "vfrete = '$vfrete_final', ";
		...
		$condicao_ped .= "modelo_ped = 'RDD', "; // TIPO DE PEDIDO RDD = DISTRIBUICAO DIRETO
   		$condicao_ped .= "inicio_sep = 'OK', "; // INICIO DA SEPARACAO
		$condicao_ped .= "avalista = '$codcliente_aval' "; // CLIENTE AVALISTA
