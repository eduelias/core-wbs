...
switch ($Action)
{
	case "start":
	 ...
	 $prod->delProd("pedidoprodtemp_micro", "codped = '$codpedselect' and prod_add <> 'OK'");
		...
		$cont_micro=0;
		//VERIFICA SE EXISTEM PRODUTOS SEPARADOS DO MICRO
		$prod->clear();
		$prod->listProdU("COUNT(*)","pedidoprodtemp_micro, produtos", "pedidoprodtemp_micro.codprod = produtos.codprod and (codcat = 72 or codcat = 73) and codped = '$codpedselect' ");
		$cont_micro = $prod->showcampo0();
		
		$prod->clear();
		$prod->listProdU("COUNT(*)","pedidoprodtemp_micro, produtos", "pedidoprodtemp_micro.codprod = produtos.codprod and (codcat = 35) and codped = '$codpedselect' ");
		$cont_note = $prod->showcampo0();
		
		$prod->clear();
		$prod->listProdU("COUNT(*)","pedidoprodtemp_micro, produtos", "pedidoprodtemp_micro.codprod = produtos.codprod and (codcat <> 65 and codcat <> 72 and codcat <> 72 and codcat <> 35 ) and codped = '$codpedselect' ");
		$cont_outros = $prod->showcampo0();
		
		if ($cont_note <> 0 and $cont_outros == 0){$cont_micro = $cont_note+$cont_micro;}
    	...
    	$prod->listProdgeral("pedidoprodtemp_micro", "codped = '$codpedselect'", $arrayx1, $array_campox1, "");
			...
			#echo "$codpedselect - micro=".$cont_micro;
			...
			#echo "micro=".$cont_micro;
			
			$erro_carrinho = "OS PRODUTOS SELECIONADOS NAO PODEM SER ADICIONADOS NO CARRINHO";
	...
	if ($pvdcj[$k] == 0){$preco_dist[$k] = $pvd[$k];}else{$preco_dist[$k] = $pvdcj[$k];}
	...
	if ($tipovend == "A" or $tipovend == "R"){$preco[$k] = $preco_dist[$k];}else{$preco[$k] = $pvvcj[$k];}
