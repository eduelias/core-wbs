   ...
   switch ($Action)
{
	case "grava":
		...
		#$vt = $valortabela_real;
		$prod->clear();
		$prod->listProdU("SUM(ppu*qtde)","pedidoprodtemp", "codped ='$codpedselect'");
		$vt_mdped = $prod->showcampo0();
...
if ($orcamento == 1){
		...
		if ($tipo_pessoa == "F"){
		        if (dirname($_SERVER['PHP_SELF']) == "/"){$bar = "";}else{$bar = "/";}
			header("Location: http://" . $_SERVER['HTTP_HOST']. dirname($_SERVER['PHP_SELF']). $bar . "restrito.php?pg=$pg&pg_ped=12&codpedselect=$codpedselect&tipo_p=$tipo_p&cidade_cli=$cidade_cli");
		}else{
			if (dirname($_SERVER['PHP_SELF']) == "/"){$bar = "";}else{$bar = "/";}
			header("Location: http://" . $_SERVER['HTTP_HOST']. dirname($_SERVER['PHP_SELF']). $bar. "restrito.php?pg=$pg&pg_ped=13&codpedselect=$codpedselect&tipo_p=$tipo_p&cidade_cli=$cidade_cli");
		}
}
