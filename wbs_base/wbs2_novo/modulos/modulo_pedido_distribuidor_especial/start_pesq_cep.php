          ...
          if ( mysql_num_rows( $result ) > 0 )
       	 {
              ...
              $sql  = "SELECT nomeloc AS cidade , cidade_atend, valor_desloc FROM localidades WHERE chaveloc='".$dados['cidade']."' ";
              $result = mysql_query ($sql);
              $pesq = mysql_fetch_array( $result );
              $dados['cidade'] = $pesq['cidade'];
              $dados['cidade_atend'] = $pesq['cidade_atend'];
              $dados['valor_desloc'] = $pesq['valor_desloc'];
       	      
              return $dados;
          } else {
			 

			 $sql  = "SELECT nomeloc AS cidade, cidade_atend, valor_desloc FROM localidades WHERE ceploc='".$cep."' ";
			  $result = mysql_query ($sql)
				   or die ('Erro na query' . mysql_error());
