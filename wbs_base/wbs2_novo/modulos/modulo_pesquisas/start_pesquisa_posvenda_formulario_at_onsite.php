...
switch ($Action)
{
    	...
    	case "Pesquisar" :
            ...
            $prod->clear();
            $prod->listProdU("COUNT(*)","os_at_analise", "codos_at = '$codos_at_pesq' and codsrv_exec = '30102' and cancel <> 'S'");
            $atend_tel_view = $prod->showcampo0();
            if ($atend_tel_view > 0){$tipo_atend =  "TELEFONE";}else{$tipo_atend =  "RESIDÊNCIA";}
            
            if ($jarespondeu_view == 0)
            {
                include ("templates/pesquisa_posvenda_formulario_at_onsite.html");
            }
            else
            {
    	...
    	case "Gravar" :
            ...
            for($i = 27; $i < 36; $i++ )
            {
                ...
                if (($i == 27 and $resp[$i] == "N") or ($i == 28 and $resp[$i] == "N") or ($i == 30 and $resp[$i] == "N") or ($i == 31 and $resp[$i] == "N") or ($i == 33 and ($resp[$i] == "PS" or $resp[$i] == "I" )) or  ($i == 35 and $resp[$i] <> "")){$oc = $oc *0;}else{$oc = $oc *1;}
            ...
            $prod->clear();
            $prod->setcampo0("");
            $prod->setcampo1("3"); // codigo modelo pesquisa
