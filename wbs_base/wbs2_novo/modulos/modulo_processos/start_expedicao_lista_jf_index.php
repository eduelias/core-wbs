...
$condicaopesq = " ((pedido.status = 'LIB') or (pedido.modped = 'OK' and pedido.cb = 'OK'  )) and (pedido.contrato = 'OK' or  pedido.contrato = 'EP' or  pedido.contrato = 'DC' ) and pedido.caixa = 'OK' and modoentr <> 'RETIRA' and endentrega like '%juiz%' and cancel <> 'OK'  AND pedido.codcliente=clientenovo.codcliente and pedido.codvend=vendedor.codvend";

$rs_lista = $db->query_db("SELECT codped, pedido.codcliente, pedido.codvend, dataped, dataprevent, status, horaprevent, nf, contrato, libentr, codbarra, caixa, clientenovo.nome as nomecli, vendedor.nome as nomevend, fat, modped, modoentr, dataprevent_old, aguard_comp, declara, prontaentr, modelo_ped, ped_transf, datasol_nf, lista_desp, dddtel1, tel1, dddtel2, tel2, endereco, numero, complemento, bairro, cep, cidade, estado, rescredito, refentrega, obsentrega, endentrega, clientenovo.tipocliente FROM pedido, clientenovo, vendedor WHERE $condicaopesq and pedido.codemp = '20' ORDER by dataprevent","SQL_NONE","N");
...
$rs_empresa = $db->query_db("SELECT codemp, nome FROM empresa WHERE codemp = '20'","SQL_NONE","N");
$empresa = $rs_empresa->fields['codemp'];

$db->disconnect();

include ("templates/expedicao_lista_jf_index.htm");
