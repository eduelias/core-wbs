...
$condicaopesq = " (pedido.datasol_nf <> 0 or (pedido.prontaentr = 'OK' and pedido.caixa = 'OK' )) and (pedido.status = 'LIB' or pedido.status = 'ENTR' or pedido.status = 'DESP')  and hist <> 1 and pedido.fat <> 'OK' AND pedido.codcliente=clientenovo.codcliente and pedido.codvend=vendedor.codvend";

$rs_lista = $db->query_db("SELECT codped, pedido.codcliente, pedido.codvend, dataped, dataprevent, status, horaprevent, nf, contrato, libentr, codbarra, caixa, clientenovo.nome as nomecli, vendedor.nome as nomevend, fat, modped, modoentr, dataprevent_old, aguard_comp, declara, prontaentr, modelo_ped, ped_transf, datasol_nf, lista_desp, dddtel1, tel1, endereco, numero, complemento, bairro, cep, cidade, estado, rescredito, datasol_nf, refentrega, obsentrega, endentrega, clientenovo.tipocliente, ie,  modelo_ped, codemp_pis FROM pedido, clientenovo, vendedor WHERE $condicaopesq and pedido.codemp = '$empresa' ORDER by prontaentr  ASC , datasol_nf","SQL_NONE","N");
...
include ("templates/faturamento_lista_index.htm");
