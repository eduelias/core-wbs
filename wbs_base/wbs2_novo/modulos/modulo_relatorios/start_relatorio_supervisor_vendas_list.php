...
switch ($Action)
{
    case "Pesquisar" :
    	...
    	if ($mes_pesq <> "" and  $ano_pesq <> "" and  $codproj_pesq <> -1) {               
			...
			$rs = $db->query_db("SELECT codvend, nome, meta FROM vendedor WHERE vendedor.codproj='$codproj_pesq' and block <> 'Y' ","SQL_NONE","N");
...
if ($codgrp_vend == 2 or $codgrp_vend == 54 or $codgrp_vend == 62 or $codgrp_vend == 79){
	...
	$condproj = "WHERE codproj = '$codproj_vend'"; 
}
...
include ("templates/relatorio_supervisor_vendas_list.htm");
