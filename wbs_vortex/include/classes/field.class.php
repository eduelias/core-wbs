...
class field extends bd{
	...
	function preenche(){
		...
		$this->tipos = array(
			...
			"calendar"=>
			'<div id="'.$this->elid.'Container"></div>
	...
	WBS.cal1 = new YAHOO.widget.Calendar("'.$this->elid.'Container", {selected:"'.date('m/d/Y',strtotime($this->valor)).'", LOCALE_WEEKDAYS:"short"});
	...
	var '.$this->elname.' = new Spry.Widget.ValidationTextField("text_'.$this->elid.'", "custom", {validateOn:["blur"], useCharacterMasking:true, pattern:"'.$this->pattern.'", isRequired:'.$this->required.'})
			...
			"textarea"=>
			"<label><br>
			<textarea  name='".$this->tabela."@".$this->elname."' id='".$this->elid."' ".$this->js." cols=".($this->tamanho*2)." rows=".($this->tamanho/5)." class=".$this->style." onKeyDown=\"javascript:textCounter(".$this->elid.",remLentext,100);\" onKeyUp=\"javascript:textCounter(".$this->elid.",remLentext,".($this->tamanho*10).");\">".$this->valor."</textarea><p>Faltam: <span id=remLentext>".($this->tamanho*10)."</span> caracteres</p>
			</label>",
			
			"richarea"=>
			"<label><br>
			<link rel=\"stylesheet\" type=\"text/css\" href=\"http://yui.yahooapis.com/2.6.0/build/assets/skins/sam/skin.css\"><textarea  name='".$this->tabela."@".$this->elname."' id='t_".$this->elid."' ".$this->js." cols=".($this->tamanho*2)." rows=".($this->tamanho/5)." class=".$this->style." onKeyDown=\"javascript:textCounter(".$this->elid.",remLentext,100);\" onKeyUp=\"javascript:textCounter(t_".$this->elid.",remLentext,".($this->tamanho*10).");\">".$this->valor."</textarea>
			</label><script>var myEditor = new YAHOO.widget.SimpleEditor('t_".$this->elid."', {
				dompath: true //Turns on the bar at the bottom
			});
			myEditor.render();
			</script><p>Faltam: <span id=remLentext>".($this->tamanho*10)."</span> caracteres</p>",
	...
	function insere($campo_id){
		...
		switch ($row_tb_form_field[0]['tipo']) {
			case 'enum': {
				$this->prepara_enum(); 
				$this->preenche();
				echo "<tr width='100%' id='".$this->elid."'><td style='padding:1px;' class='td_lab'>".$this->label.": </td><td style='padding:1px;' class='td_field'><li id='".$this->elid."'>".$this->tipos[$row_tb_form_field[0]['tipo']]."</li></td></tr>";
			} break;
			case 'select':{
