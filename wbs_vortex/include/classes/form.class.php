...
class form extends bd {
	...
	function imprime($form_id,$recordID){
		...
		$row_tb_form_field = parent::gera_array('frm_formulario.*,frm_formulario.mth as method','frm_formulario','idformulario ='.$form_id);
		
		$cap_submit = $row_tb_form_field[0]['cap_submit'];
		$cap_reset = $row_tb_form_field[0]['cap_reset'];
		echo "<form action='".$row_tb_form_field[0]['action']."' ".$row_tb_form_field[0]['js']." method=".$row_tb_form_field[0]['method']." ".$row_tb_form_field[0]['js']." class='".$row_tb_form_field[0]['style']."' ><fieldset><legend>".$row_tb_form_field[0]['titulo']."</legend>";
		...
		if (is_array($tb)) { 
			...
			erro('PROBLEMAS NA CONSULTA SQL, RETORNADO 0 >SQL> '.parent::get_sql());
		}		
