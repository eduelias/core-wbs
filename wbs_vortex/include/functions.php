...
###################################################################
	...
	function showabas($pgid, $modulo) {
		...
		foreach($lista as $list){
				...
				closer: false,
				id: ".$i.",
				dataSrc: 'ajax_html.php?file=".encode("modulos/".$list['arquivo']."/".$list['sub_arquivo']."'").", \r\n";
...
function GeraBarcode($barcode) {
 ...
 define (__DEBUG_ENABLED__, false);
	...
	if ($IP_S == '200.251.60.6') { 
		require_once("/var/www/wbs/wbs_base/gerabarcode/barcode.php");		   
 		require_once("/var/www/wbs/wbs_base/gerabarcode/c128bobject.php");			   
	} else {
 		require_once("/home/wbs/public_html/maxxmicro/wbs_base/gerabarcode/barcode.php");
 		require_once("/home/wbs/public_html/maxxmicro/wbs_base/gerabarcode/c128bobject.php");
	}
...
if (isset($barcode) && strlen($barcode)>0) {    
  ...
  $obj = new C128BObject(250, 100, $style, $barcode);
	
  if ($obj) {
     if ($obj->DrawObject($xres)) {
         return "<img src='../wbs_base/gerabarcode/image.php?code=".$barcode."&style=".$style."&type=".$type."&width=".$width."&height=".$height."&xres=".$xres."&font=".$font."'>";
     } else return "<font color='#FF0000'>".($obj->GetError())."</font>";
  }
}
