<div id="cadastro">
    ...
    <span id="cadastro_sec">
       <label for="nome">NOME:</label><input type="text" id="nome" /><br />
        ...
        <label for="uf">UF:</label><select id="uf"><?php include('modulos/mod_pedido/pedido_rapido/cliente_estados.php');?></select>CIDADE: <select id="cidade"><option value="Juiz de Fora">Juiz de Fora</option></select> 
...
<script>
	...
	VENDA.procura_cliente = function(oSelf,campo) {
		...
		if (oSelf.value != '') {
			...
			var file = '<?=encode('modulos/mod_pedido/pedido_rapido/ajax_cliente.php')?>';
	...
	YAHOO.util.Event.addListener(cpf, "blur", function(e, oSelf) { 
				VENDA.procura_cliente(oSelf,'cpf');
	},cpf);
