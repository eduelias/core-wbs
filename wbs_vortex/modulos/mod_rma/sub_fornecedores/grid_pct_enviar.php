...
<script>
	...
	YAHOO.widget.DataTable.prototype.formatEnvia = function (elCell,oRecord,oColumn,oData) {
		...
		var url = "ajax_html.php?file=<?=encode('modulos/mod_rma/ajax/ajax_pct_envio.php')?>&id="+oData+"&tipo=6";
		var oTable = this;
		...
		if((nf!='000000')||(nf=='BALCAO')) {
			...
			YAHOO.util.Event.addListener(elCell, "click", function(e, oSelf) {
				...
				if (confirm('PACOTE ENVIADO PARA FORNECEDOR?','SIM','NAO')) {
...
<?
	...
	$fmt_detalhes = new formatter(array(tipo=>"det_tab",arquivo=>"modulos/mod_rma/sub_revenda/grid_rma_detalhe.php", args=>array('modulo'=>'rma_fornecedor', mensagem=>'Detalhes do pacote')));
	...
	$tabelas = 'v_rma_pct,fornecedor';
	
	$condicao = "v_rma_pct.short_desc = fornecedor.codfor and v_rma_pct.tipo = 6 and v_rma_pct.hist=\'N\'";
